...
from .config import BPTT_BACKEND, V24IlyaTrainConfig
...
class V24IlyaTrainingTransforms:
...
def build_training_transforms(
    model_configs: V24IlyaModelConfigs,
    task_config,
    stats,
    ...
    return V24IlyaTrainingTransforms(
...
def cast_physical_boundary_fp32(tree):
    """Keep raw physical inputs, targets, forcings, and feedback in FP32."""
    return _cast_floating(tree, jnp.float32)
...
def residual_target(truth, baseline_prediction):
    truth = _cast_floating(truth, jnp.float32)
    baseline_prediction = _cast_floating(baseline_prediction, jnp.float32)
...
def feedback_field(mode: str, baseline_prediction, residual_prediction):
    baseline_prediction = _tree_stop(
        _cast_floating(baseline_prediction, jnp.float32)
    )
    if mode == "baseline":
        return baseline_prediction
    if mode == "closed_loop_sg":
        return _cast_floating(jax.tree_util.tree_map(
            lambda baseline, residual: baseline + jax.lax.stop_gradient(residual),
            baseline_prediction,
            _cast_floating(residual_prediction, jnp.float32),
        ), jnp.float32)
...
def next_dynamic_frame(
    ...
    *,
    ...
    state = _cast_floating(field, jnp.float32).assign_coords(time=target_time)
    forcing = _cast_floating(forcing, jnp.float32).assign_coords(time=target_time)
...
def make_bptt_objective(
    *,
    ...
    if input_steps != 2:
        raise ValueError("v24_Ilya explicit reverse BPTT requires input_steps=2")
    ...
    def objective(
        ...
        initial_state = cast_state_boundary_fp32(initial_state)
        input_frames = tuple(
            _cast_floating(frame, jnp.float32) for frame in input_frames
        )
        static_inputs = _cast_floating(static_inputs, jnp.float32)
        truths = tuple(_cast_floating(truth, jnp.float32) for truth in truths)
        forcings = tuple(_cast_floating(forcing, jnp.float32) for forcing in forcings)
...
def make_train_step(
    *,
    ...
    if input_steps != 2:
        raise ValueError("v24_Ilya streaming BPTT requires input_steps=2")
    if jax.tree_util.tree_leaves(baseline_state):
