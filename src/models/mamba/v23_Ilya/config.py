"""Configuration and CLI contract for v23_Ilya evaluation."""
...
class V23IlyaArchitectureConfig:
    """Architecture shared by v23_Ilya training and evaluation."""
...
@dataclass(frozen=True)
class V23IlyaEvalConfig:
    """Complete, validated configuration for one v23_Ilya evaluation."""
    ...
    @property
    def architecture(self) -> V23IlyaArchitectureConfig:
        return V23IlyaArchitectureConfig(
...
def parse_args(argv: Sequence[str] | None = None) -> V23IlyaEvalConfig:
    namespace = build_arg_parser().parse_args(argv)
    return V23IlyaEvalConfig(**vars(namespace))
