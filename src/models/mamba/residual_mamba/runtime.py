    ...
    *,
    ...
    def run_jitted(**kw):
        ...
        for step_i in range(targets_template.sizes["time"]):
            ...
            # v22-style: residual head sees absolute state (same as baseline),
            # NOT residual_inputs (which we keep around only for legacy/state tracking).
...
def build_training_equivalent_run_jitted(ckpt_obj, stats, ckpt_path: Path):
    ...
    def run_jitted(**kw):
        ...
        for step_i in range(targets.sizes["time"]):
            ...
            # v22-style: residual head sees absolute state.
            residual_pred, residual_state = residual_bundle["step"](
                rng=step_keys[2 * step_i + 1],
                inputs=all_inputs,
                targets_template=target_step,
...
def build_truth_anchored_residual_runner(
    ...
    *,
    ...
    def _model_step(*, rng, context, target_step, forcings_step, teacher_forced_residual: bool):
        ...
        # v22-style: residual head sees absolute state (same all_inputs as baseline).
        residual_pred, residual_state = residual_bundle["step"](
            rng=rng[1],
            inputs=all_inputs,
            targets_template=target_step,
