...
def run_training(
    ...
    *,
    ...
    # Pre-build per-var stddev arrays for normalized feedback clipping (V-B).
    # Subset to task_cfg.pressure_levels (stats files carry all 37 levels but
    # the task config uses a subset). Captured by closures below as a constant
    # (jit-safe).
    feedback_var_stddevs = make_feedback_var_stddev_dict(
        ...
        def train_chunk(
            ...
            def loss_fn(p, s, key):
                ...
                def one_bptt_step(
                    ...
                    # v22-style: residual head takes ABSOLUTE state (same input
                    # window as baseline GraphCast), NOT zero-init residual
                    # values. Matches train_mz_v20.py:404-405 where both
                    # baseline_predict and residual_predict are called with
                    # the same `current_inputs` (absolute weather state).
                    ...
                    # Closed-loop consistency: when the physical rolling-input
                    # stream uses a clipped/scaled residual, the Mamba memory
                    # stream must use the SAME residual (= λ * processed_residual),
                    # not the raw residual_preds. See feedback_closed_loop_consistency
                    # memory + residual_feedback_components().
                    if use_truth_residual_feedback:
                        residual_for_memory = residual_targets
                        feedback_preds = None  # not needed during truth prefix
                    else:
                        ...
                        feedback_preds, gated_residual_used = residual_feedback_components(
                            ...
                            feedback_clip_mode=segment_cfg.feedback_clip_mode,
                            feedback_clip_c=segment_cfg.feedback_clip_c,
                            feedback_grad_scale=segment_cfg.feedback_grad_scale,
                            var_stddevs=feedback_var_stddevs,
                        )
                        # mode='baseline' returns gated_residual_used=None — in that
                        # open-loop case Mamba memory still needs to evolve from its
                        # own self-prediction, so fall back to raw residual_preds.
                        residual_for_memory = (
                    ...
                    if update_tail_rolling_inputs:
                        if feedback_preds is None:
                            # truth boundary: bptt_i still in prefix but next step is tail.
                            full_preds = baseline_preds + residual_preds
