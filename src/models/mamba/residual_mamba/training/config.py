...
class ResidualSegmentRunConfig:
    ...
    # === Version B: feedback-only normalized clipping ===
    # When residual_ar_feedback='gated', clip the per-variable residual that
    # gets fed back into the frozen GraphCast input, in normalized (input-std)
    # units. Output residual is NOT clipped — only the feedback path.
    feedback_clip_mode: str = FEEDBACK_CLIP_MODE_NONE
    feedback_clip_c: float = 1.0
    # Gradient flow through the feedback path. 1.0 = full gradient (default).
    # 0.0 = stop_gradient (= legacy stop_grad_feedback=True).
    # In (0, 1) = partial credit assignment (V-B2).
    # `stop_grad_feedback=True` overrides this to 0.0.
...
def parse_args(argv: list[str] | None = None) -> ResidualSegmentRunConfig:
    ...
    parser.add_argument("--feedback-clip-mode", choices=FEEDBACK_CLIP_MODE_CHOICES,
        default=FEEDBACK_CLIP_MODE_NONE,
        help=(
            "(V-B) Clip the residual fed back into baseline input, in normalized "
            "(input-std) units. 'none' = no clipping (V-A behavior). 'tanh' = "
            "c*tanh(r/c). 'hard' = clip(r, -c, c). Output residual is NOT clipped."
        ))
    parser.add_argument("--feedback-clip-c", type=float, default=1.0,
        help="(V-B) Clip threshold c in normalized units (e.g. 0.5, 1.0). Used by --feedback-clip-mode.")
    parser.add_argument("--feedback-grad-scale", type=float, default=1.0,
        help=(
            "(V-B2) Scale gradient flowing back through feedback path. 1.0 = full "
            "(default; matches V-A when --stop-grad-feedback is off). 0.0 = full stop. "
            "(0, 1) = partial credit assignment. --stop-grad-feedback overrides to 0.0."
        ))
    ...
    if args.feedback_clip_c <= 0.0:
        raise ValueError("--feedback-clip-c must be > 0")
    if not (0.0 <= args.feedback_grad_scale <= 1.0):
        raise ValueError("--feedback-grad-scale must be in [0, 1]")
    if args.feedback_clip_mode != FEEDBACK_CLIP_MODE_NONE and args.residual_ar_feedback != "gated":
        raise ValueError(
            f"--feedback-clip-mode={args.feedback_clip_mode!r} only applies when "
    ...
    return ResidualSegmentRunConfig(
        ...
        feedback_clip_mode=args.feedback_clip_mode,
        feedback_clip_c=args.feedback_clip_c,
        feedback_grad_scale=args.feedback_grad_scale,
