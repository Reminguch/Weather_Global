...
def should_checkpoint_residual_ar_step(memory_mode: str) -> bool:
    return memory_mode in ("conservative", "optimal")
...
def build_loss_transform(
    ...
    def forward_fn(inputs, targets, forcings, is_training):
        training_memory_mode = getattr(cfg, "memory_mode", "standard") if bool(is_training) else "standard"
...
def residual_autoregressive_final_horizon(
    ...
    *,
    ...
    stop_baseline_gradient: bool = False,
    checkpoint_step: bool = False,
    feedback_lambda: jax.Array | None = None,
    ...
    def one_ar_step(
        ...
        full_inputs = xr.merge([constant_inputs, rolling_inputs_step])
        ...
        # v22-style: residual head sees same absolute state as baseline (full_inputs).
        (loss_and_diag, residual_preds), next_residual_state = residual_loss_prediction_transform.apply(
            residual_params,
            current_residual_state_step,
            residual_key,
            full_inputs,
        ...
        feedback_preds, gated_residual_used = residual_feedback_components(
            baseline_pred=baseline_preds,
            full_pred=full_preds,
            mode=residual_ar_feedback,
            lam=feedback_lambda,
            stop_grad=stop_grad_feedback,
        ...
        # Closed-loop stream consistency: Mamba memory uses the SAME residual
        # that the physical rolling input received. For open-loop (mode='baseline',
        # gated_residual_used=None) fall back to raw residual_preds so Mamba's
        # self-prediction signal still propagates.
        residual_for_memory = (
            gated_residual_used if gated_residual_used is not None else residual_preds
        )
        next_residual_inputs = advance_residual_inputs(current_residual_inputs_step, residual_for_memory)
        teacher_next_residual_inputs = advance_residual_inputs(current_residual_inputs_step, residual_targets)
        next_rolling_inputs = _update_inputs(rolling_inputs_step, xr.merge([feedback_preds, forcings_step]))
        return (
            xarray_jax.unwrap_data(loss_and_diag[0]),
            next_residual_state,
            next_baseline_state,
            next_residual_inputs,
            teacher_next_residual_inputs,
            next_rolling_inputs,
        )

    one_ar_step_fn = jax.checkpoint(one_ar_step) if checkpoint_step else one_ar_step
    ...
    for step_i in range(targets.sizes["time"]):
        ...
        if step_i == 0:
            ...
            teacher_carry_residual_inputs = teacher_next_residual_inputs
        if step_i == targets.sizes["time"] - 1:
            final_loss_by_lane = loss_by_lane
...
def augment_run_config(
    ...
    *,
    ...
    memory_mode = getattr(segment_cfg.base_cfg, "memory_mode", "standard")
    ...
    payload["residual_training"] = {
        ...
        "autoregressive_feedback_mode": residual_ar_feedback,
        "bptt_residual_carry": (
        ...
        "baseline_stop_gradient": memory_mode in ("conservative", "optimal"),
        "residual_step_checkpoint": should_checkpoint_residual_ar_step(memory_mode),
            ...
            if rolling_ar:
                ...
                def eval_chunk(
                    ...
                    for bptt_i in range(len(chunk_inputs)):
                        ...
                        # v22-style: residual head takes absolute state, same as baseline.
                        ...
                        if bptt_i < len(chunk_inputs) - 1:
                            # Stream consistency: when bptt_i is in AR-tail and not
                            # truth-forced, advance Mamba memory with the SAME
                            # residual that the physical rolling input receives.
                            if bptt_i < truth_prefix_steps:
                                # truth-forced step: feedback = truth residual
                                ...
                                if bptt_i + 1 < truth_prefix_steps:
                                    ...
                                    # boundary: next step is AR-tail, prepare feedback_preds for rolling input
                                    full_preds = reconstruct_full_predictions(baseline_preds, residual_preds)
                                ...
                                # AR-tail closed-loop step: compute consistent (physical, memory) pair
                                full_preds = reconstruct_full_predictions(baseline_preds, residual_preds)
                                feedback_preds, gated_residual_used = residual_feedback_components(
                                    baseline_pred=baseline_preds,
                                    full_pred=full_preds,
                                ...
                                residual_for_memory = (
                                    gated_residual_used if gated_residual_used is not None
                                    else residual_preds
                                )
                                current_residual_inputs = advance_residual_inputs(
                                    current_residual_inputs,
                ...
                def eval_chunk(
                    ...
                    for bptt_i in range(len(chunk_inputs)):
                        loss_by_lane, current_state, current_residual_inputs = residual_autoregressive_final_horizon(
                            ...
                            feedback_lambda=feedback_lambda,
                            stop_grad_feedback=stop_grad_feedback,
                            feedback_clip_mode=feedback_clip_mode,
                            feedback_clip_c=feedback_clip_c,
                            feedback_grad_scale=feedback_grad_scale,
                            var_stddevs=var_stddevs,
