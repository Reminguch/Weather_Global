...
class MZResidualFullMambaMeshed(hk.Module):
    ...
    def rollout_ar(
        ...
        *,
        ...
        initial_h_states: list[jax.Array] | None = None,
        return_final_h_states: bool = False,
        ...
        # Warm-start support: caller may pass pre-computed h_states (e.g. from a
        # warm-up phase that ran the SSM over W observed cycles before the
        # forecast launch time). Default zero-init preserves cold-start
        # behaviour and byte-level back-compat with existing eval / training.
        if initial_h_states is not None:
            if len(initial_h_states) != self.cfg.layers:
                raise ValueError(
                    f"initial_h_states len={len(initial_h_states)} != "
                    f"cfg.layers={self.cfg.layers}"
                )
            h_states = [h.astype(dtype) for h in initial_h_states]
        else:
            h_states = [
        ...
        if return_final_h_states:
            return preds_tbpf, h_states
        return preds_tbpf
