...
def parse_args():
    ...
    p.add_argument("--feedback-mode", choices=["cold_bp", "cold_full"], default="cold_bp",
                   help="cold_bp = baseline-only feedback (v22 open-loop training match); "
                        "cold_full = baseline+residual feedback (v22cl closed-loop match).")
...
def main():
    ...
    # cold_full uses two branches (mirrors eval_v22_clean.py):
    #   baseline branch: pure GC self-rollout (reported as "baseline")
    #   full branch:     baseline+residual fed back (reported as "full")
    # cold_bp uses one branch: baseline drives feedback for both baseline & residual.
    baseline_traj = []
    full_traj = []
    rng_chain = rng
    if cfg.feedback_mode == "cold_full":
        cur_b = sample_inputs
        bs_b = baseline_state_init
        cur_f = sample_inputs
        bs_f = baseline_state_init
        rs_f = residual_state_init
        for k in range(K):
            ...
            rp_f, rs_f = _residual_step(residual_params, rs_f, kk_r, cur_f, tgt_k, frc_k)
            ...
            if (k + 1) % 10 == 0:
                print(f"[city-trace] step {k+1}/{K} (cold_full two-branch)")
    else:
        cur = sample_inputs
        ...
        for k in range(K):
            ...
            if (k + 1) % 10 == 0:
                print(f"[city-trace] step {k+1}/{K} (cold_bp)")

    baseline_pred = xr.concat(baseline_traj, dim="time")
    full_pred = xr.concat(full_traj, dim="time")

    # Global sanity diagnostic — spatial mean/std/rms at select leads
    for k in [0, 19, 27, 39]:
        for var in VARS:
            b = np.asarray(baseline_pred[var].isel(time=k).values).squeeze()
            f = np.asarray(full_pred[var].isel(time=k).values).squeeze()
            t = np.asarray(sample_targets[var].isel(time=k).values).squeeze()
            print(f"[global k={k:2d} {var:<26s}] "
                  f"truth mean={t.mean():+7.2f}  base mean={b.mean():+7.2f}  full mean={f.mean():+7.2f}  "
                  f"base RMSE={np.sqrt(((b-t)**2).mean()):7.3f}  full RMSE={np.sqrt(((f-t)**2).mean()):7.3f}")
