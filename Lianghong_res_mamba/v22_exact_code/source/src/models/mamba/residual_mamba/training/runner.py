...
def run_training(
    ...
    *,
    ...
    # Inherit edge normalization factor from DeepMind ckpt (its value is 0.618...);
    # forcing None changes the encoder MLP input channel count and breaks overlay.
    ...
    if cfg.resume_step is not None:
        ...
        # Overlay baseline GraphCast weights into the residual model's GC layers.
        # This way the residual model's GC part starts from the DeepMind public
        # ckpt (instead of fresh random init), and we can freeze it via
        # trainable_part=mamba below. Mamba layers (no match in baseline_ckpt)
        # remain at their fresh init.
        from src.models.mamba.training.param_utils import overlay_matching_params
        params, overlay_stats = overlay_matching_params(params, baseline_ckpt.params)
        print(
            f"Residual model: overlaid {overlay_stats.copied} GC params from baseline ckpt, "
            f"{overlay_stats.initialized} additional params left at fresh init (Mamba)."
        )
    ...
    # Optimiser: optionally freeze GraphCast inside residual model and only
    # train Mamba (--trainable-part mamba). Mirror v3/v4 setup so apples-to-apples
    # vs frozen-GC baselines.
    trainable_part = getattr(segment_cfg, "trainable_part", "all")
    if trainable_part == "all":
        opt = optax.adamw(cfg.lr, weight_decay=cfg.weight_decay)
    else:
        from src.models.mamba.training.param_utils import build_trainable_labels
        labels = build_trainable_labels(params, trainable_part)
        ...
        print(f"[trainable] part={trainable_part}  train={n_train}  freeze={n_freeze}")
