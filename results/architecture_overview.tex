...
%=============================================================================
\section{The Sampling Problem: Why Mamba Cannot Learn Long-Range Memory}
%=============================================================================

\subsection{Current Training: Random Sampling with State Reset}

The current training procedure draws random time windows from 2020--2021:
...
\subsection{Proposed Solution: Sequential Sampling with Truncated BPTT}

To give Mamba long continuous sequences while maintaining training stability, we propose \textbf{chunked sequential sampling}:
...
GraphCast's memoryless autoregressive design is a clear architectural limitation for long-range forecasting. We attempted to address this with a Mamba SSM that carries hidden state across rollout steps. After correcting encoding path confounds, the residual memory architecture shows that Mamba does not significantly harm the model (gap reduced from $-$20\% to $\pm$3\%), but does not consistently help either.

Our analysis identifies the \textbf{state reset due to random sampling} as the most likely bottleneck: Mamba's state is zeroed every 2--8 steps, preventing it from learning long-range temporal patterns. The proposed chunked sequential sampling with truncated BPTT would give Mamba 120-step continuous sequences while maintaining training stability and partial i.i.d.\ properties. This represents the most direct test of whether temporal memory can benefit weather forecasting at this model scale.
