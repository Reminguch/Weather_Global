...
class DeepTypedGraphNet(hk.Module):
  ...
  * num_message_passing_steps = N, num_processor_repetitions = M, gives
  ...
  def __init__(self,
    ...
    Args:
      ...
      aggregate_normalization: An optional constant that normalizes the output
        ...
        segment_mean.
      temporal_backbone: Optional temporal processor for interleaved message
  ...
  def _maybe_temporal_process_step(
      self,
    ...
    if self._temporal_backbone == "none":
      return latent_graph
    if self._temporal_location != "mesh_processor_interleaved":
      return latent_graph

    updated_nodes = {}
    changed = False
    for node_set_name, node_set in latent_graph.nodes.items():
      features = node_set.features
      if features.ndim != 4:
        updated_nodes[node_set_name] = node_set
        continue
      changed = True
      features_tnbd = jnp.transpose(features, (2, 0, 1, 3))
      features_tnbd = TemporalMeshBlock(
          TemporalMeshConfig(
      ...
      )(features_tnbd, is_training=False)
      updated_nodes[node_set_name] = node_set._replace(
          features=jnp.transpose(features_tnbd, (1, 2, 0, 3)))

    if not changed:
      return latent_graph
    return latent_graph._replace(nodes=updated_nodes)
