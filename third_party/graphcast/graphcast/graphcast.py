...
class GraphCast(predictor_base.Predictor):
  ...
  * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
  ...
  def __init__(self, model_config: ModelConfig, task_config: TaskConfig):
    ...
    self._temporal_init_scheme = "legacy_haiku"
    self._temporal_dt_init = "random"
    self._temporal_dt_min = 0.001
    self._temporal_dt_max = 0.1
    self._temporal_dt_scale = 1.0
    self._temporal_dt_init_floor = 1e-4
  ...
  def _run_mesh_gnn(
    ...
    mesh_node_features = latent_mesh_nodes
    ...
    nodes = nodes._replace(features=mesh_node_features)
      ...
      for step_i, processor_network in enumerate(processor_networks):
        ...
        temporal_block = _get_temporal_block_cls(
            ...
            TemporalMeshConfig(
                ...
                init_scheme=getattr(self, "_temporal_init_scheme", "legacy_haiku"),
                dt_init=getattr(self, "_temporal_dt_init", "random"),
                dt_min=getattr(self, "_temporal_dt_min", 0.001),
                dt_max=getattr(self, "_temporal_dt_max", 0.1),
                dt_scale=getattr(self, "_temporal_dt_scale", 1.0),
                dt_init_floor=getattr(self, "_temporal_dt_init_floor", 1e-4),
