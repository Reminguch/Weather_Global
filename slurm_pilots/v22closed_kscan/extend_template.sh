#!/usr/bin/env bash
# Template: extend a v22closed K-job by N steps via resume.
# Substitute K, START_STEP, MAX_STEPS, OLDDIR, NEWDIR, PARENT_JID before sbatch.
#SBATCH --job-name=v22closed-sg-K{K}-ext{MAX_STEPS}
#SBATCH --account=biao
#SBATCH --gres=gpu:1
#SBATCH --qos=gpu
...
#SBATCH --output=/home/lm8598/Weather_Global_experiments/logs/v22closed_sg_K{K}_ext{MAX_STEPS}_%j.out
#SBATCH --error=/home/lm8598/Weather_Global_experiments/logs/v22closed_sg_K{K}_ext{MAX_STEPS}_%j.err
#SBATCH --dependency=afterok:{PARENT_JID}
set -euo pipefail
...
# RESUME path: read step (START_STEP-1) ckpt from prior run
RESUME_CKPT="{OLDDIR}/v13_residual_step{RESUME_FROM_STEP}.pkl"
OUT_BASE=/scratch/gpfs/DABANIN/lm8598/Weather_Global/results/v22closed
OUT="${OUT_BASE}/K{K}_sg_fresh_{MAX_STEPS}"
...
${PYTHON} -u ${SCRIPT} \
  ...
  --out-dir "${OUT_BASE}" \
  --run-name K{K}_sg_fresh_{MAX_STEPS} \
  ...
  --batch-size 1 \
  --max-steps {MAX_STEPS} --checkpoint-every 1000 \
