"""v21 = v20 + FREEZE residual stream's grid2mesh_gnn AND mesh2grid_gnn.

Ablation: are the residual encoder/decoder drifts (away from GC1 weights)
actually useful, or does freezing them help (less overfitting, fewer params,
faster)? In v21 only Mamba + residual mesh_gnn (shallow processor) + zero-init
head are trainable; grid2mesh and mesh2grid stay at GC1-overlaid init.

Implementation: optax.masked. Mask = True for params we update, False for
grid2mesh_gnn and mesh2grid_gnn blocks.

Rest of training (FIXED AR forcings) is identical to v20.
...
def main():
    ...
    if cfg.warmup_steps > 0:
        ...
        lr_schedule = cfg.lr
    # v21: freeze grid2mesh_gnn and mesh2grid_gnn via optax.multi_transform.
    # Build a label tree (same structure as residual_params) labeling each
    # module 'trainable' or 'frozen'. Trainable -> AdamW. Frozen -> set_to_zero.
    ...
    print(f"[v21] FREEZE grid2mesh + mesh2grid via multi_transform+set_to_zero:")
    print(f"[v21]   trainable params: {n_train:,} ({100*n_train/(n_train+n_frozen):.1f}%) "
          f"across {len(trainable_modules)} modules")
    print(f"[v21]   frozen params:    {n_frozen:,} ({100*n_frozen/(n_train+n_frozen):.1f}%) "
          f"across {len(frozen_modules)} modules")
    print(f"[v21]   total residual params: {n_train+n_frozen:,}")
    print(f"[v21]   first 3 frozen modules: {[m[0] for m in frozen_modules[:3]]}")
    print(f"[v21]   first 3 trainable modules: {[m[0] for m in trainable_modules[:3]]}")

    # Optimizer: clip → multi_transform({trainable: AdamW, frozen: set_to_zero}).
    ...
    print(f"[v21] opt: grad_clip={cfg.grad_clip}, warmup={cfg.warmup_steps}, "
