"""WB2-GRID VARIANT: scores on 1.5-deg conservative-regridded fields.
...
import numpy as np
import xarray as xr
from wb2_conservative_regrid import regrid_dataset as _wb2_regrid, TGT_LAT as _WB2_LAT
...
def main():
    ...
    # WB2-grid scoring: weights on the 1.5-deg verification grid
    lat = _WB2_LAT
    ...
    for s_i, idx in enumerate(chosen_idx):
        ...
        # WB2 protocol: conservative-regrid forecasts AND truth to 1.5 deg
        # (240x121) BEFORE computing any metric.
        baseline_pred = _wb2_regrid(baseline_pred)
        full_pred = _wb2_regrid(full_pred)
        tgt_eval = _wb2_regrid(tgt_eval)
