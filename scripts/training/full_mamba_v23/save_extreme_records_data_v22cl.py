...
# Pressure-level slices to extract for TC tracking (TempestExtremes needs
# geopotential at 300 & 500 hPa for warm-core / thickness check).
# Stored in output as e.g. `geopotential_300_truth`, `geopotential_500_v18`.
PLEV_VARS = [("geopotential", 300), ("geopotential", 500)]
...
def parse_args():
    ...
    # v22cl closed-loop support
    p.add_argument("--feedback-mode", choices=["cold_bp", "cold_full"], default="cold_bp",
                   help="cold_bp: baseline-only feedback (v20/v22 open-loop). "
                        "cold_full: baseline+residual feedback (v22cl closed-loop, two-branch).")
    p.add_argument("--residual-state-init", choices=["ckpt", "zero"], default="ckpt",
                   help="ckpt: load residual_state from ckpt if present (v20 legacy). "
                        "zero: force zero-init (matches SWA global eval convention).")
    # TCBench-style: explicit init list, robust to disjoint-time zarrs
    p.add_argument("--anchor-times", default=None,
                   help="Comma-separated ISO init times (overrides the default 2022 "
                        "3-day-stride pool). Example: 2023-07-24T12,2023-08-30T00")
    p.add_argument("--anchor-year", type=int, default=2022,
                   help="Year to build default 3-day-stride anchor pool when "
                        "--anchor-times is not set.")
    return p.parse_args()
...
def main():
    ...
    if cfg.anchor_times:
        anchor_times = [pd.Timestamp(s.strip()) for s in cfg.anchor_times.split(",")]
        print(f"Using explicit --anchor-times: {len(anchor_times)} times")
    else:
        anchor_times = pd.date_range(f"{cfg.anchor_year}-01-02 00:00",
                                     f"{cfg.anchor_year}-12-30 00:00",
                                     freq=f"{cfg.anchor_stride_days}D")
    valid_anchors = []
    times_ns = ds.time.values.astype("datetime64[ns]")
    times_set = set(times_ns.astype("i8"))
    for t in anchor_times:
        t_ns = np.datetime64(t, "ns")
        ok = True
        for k in range(-(input_steps-1), K+1):
            tk = t_ns + np.timedelta64(int(k*6*3600*1e9), "ns")
            if int(tk.astype("i8")) not in times_set:
                ok = False; break
        if ok:
            valid_anchors.append(t_ns)
    print(f"valid_anchors: {len(valid_anchors)}"
          f" (stride={cfg.anchor_stride_days}d)" if not cfg.anchor_times else "")

    rng = jax.random.PRNGKey(0)
    sample_t = valid_anchors[0]
    sample_stamps = [pd.Timestamp(sample_t) + pd.Timedelta(hours=int(k*6))
                     for k in range(-(input_steps-1), 2)]
    sample_stamps_np = np.array([np.datetime64(s, "ns") for s in sample_stamps])
    sub = ds.sel(time=sample_stamps_np)
    ...
    if cfg.residual_state_init == "ckpt" and "residual_state" in ck and ck["residual_state"]:
        residual_state = ck["residual_state"]
        print(f"[v22cl-extreme] residual_state loaded from ckpt")
    else:
        print(f"[v22cl-extreme] residual_state = zero (init from hk.init)")
    print(f"[v22cl-extreme] feedback_mode = {cfg.feedback_mode}  ckpt = {cfg.ckpt}")
    ...
    def build_anchor_batch(t):
        # Robust to disjoint-time zarrs (e.g., a mini-zarr containing multiple
        # non-contiguous TCBench init windows): select by absolute stamp, not by
        # positional index. sel(...) enforces exact match; drop_duplicates
        # protects against multi-init overlap in edge cases.
        stamps = [pd.Timestamp(t) + pd.Timedelta(hours=int(k * 6))
                  for k in range(-(input_steps - 1), K + 1)]
        stamps_np = np.array([np.datetime64(s, "ns") for s in stamps])
        sub = ds.sel(time=stamps_np)
        # If build_single_sample uses positional slice inside, we already handed
        # it a contiguous per-init slice, so its default indexing is safe here.
    ...
    # Pressure-level slices (key = f"{var}_{level}", e.g. "geopotential_300")
    plev_truth = {f"{v}_{L}": np.full((n_anc, K, nlat, nlon), np.nan, dtype=np.float32) for v, L in PLEV_VARS}
    plev_base  = {f"{v}_{L}": np.full((n_anc, K, nlat, nlon), np.nan, dtype=np.float32) for v, L in PLEV_VARS}
    plev_full  = {f"{v}_{L}": np.full((n_anc, K, nlat, nlon), np.nan, dtype=np.float32) for v, L in PLEV_VARS}

    t_start = time.time()
    for i_anc, t_anc in enumerate(valid_anchors):
        inp, tgt, frc = build_anchor_batch(t_anc)
        rng_local, _ = jax.random.split(rng)
        if cfg.feedback_mode == "cold_full":
            # Two branches: baseline self-rollout for "baseline" metric,
            # closed-loop (bp+rp) trajectory for "full" metric.
            cur_b = inp                # baseline branch
            cur_f = inp                # full branch
            rs_f = residual_state
            for k in range(K):
                tgt_k = tgt.isel(time=slice(k, k+1))
                frc_k = frc.isel(time=slice(k, k+1))
                rng_local, rng_b, rng_r = jax.random.split(rng_local, 3)
                bp_b = _baseline_step(baseline_params, baseline_state, rng_b, cur_b, tgt_k, frc_k)
                bp_f = _baseline_step(baseline_params, baseline_state, rng_b, cur_f, tgt_k, frc_k)
                rp_f, rs_f = _residual_step(residual_params, rs_f, rng_r, cur_f, tgt_k, frc_k)
                full = jax.tree_util.tree_map(lambda b, r: b + r, bp_f, rp_f)

                for v in SURF_VARS:
                    if v in tgt_k.data_vars:
                        out_truth[v][i_anc, k] = _to_latlon(tgt_k[v].isel(time=0, batch=0))
                        out_base[v][i_anc, k]  = _to_latlon(bp_b[v].isel(time=0, batch=0))
                        out_full[v][i_anc, k]  = _to_latlon(full[v].isel(time=0, batch=0))
                for v, L in PLEV_VARS:
                    key = f"{v}_{L}"
                    if v in tgt_k.data_vars and "level" in tgt_k[v].dims:
                        plev_truth[key][i_anc, k] = _to_latlon(tgt_k[v].sel(level=L).isel(time=0, batch=0))
                        plev_base[key][i_anc, k]  = _to_latlon(bp_b[v].sel(level=L).isel(time=0, batch=0))
                        plev_full[key][i_anc, k]  = _to_latlon(full[v].sel(level=L).isel(time=0, batch=0))

                if k < K - 1:
                    next_frc = frc.isel(time=slice(k, k+1))
                    cur_b = _shift_inputs(cur_b, bp_b, next_frc)
                    cur_f = _shift_inputs(cur_f, full, next_frc)
        else:
            # cold_bp: original v20 open-loop rollout (single trajectory, baseline feedback).
            rs = residual_state
            current_inp = inp
            for k in range(K):
                tgt_k = tgt.isel(time=slice(k, k+1))
                ...
                for v, L in PLEV_VARS:
                    ...
                    if v in tgt_k.data_vars and "level" in tgt_k[v].dims:
                        plev_truth[key][i_anc, k] = _to_latlon(tgt_k[v].sel(level=L).isel(time=0, batch=0))
                        plev_base[key][i_anc, k]  = _to_latlon(bp[v].sel(level=L).isel(time=0, batch=0))
                        plev_full[key][i_anc, k]  = _to_latlon(full[v].sel(level=L).isel(time=0, batch=0))
    ...
    for v, L in PLEV_VARS:
        key = f"{v}_{L}"
        out_ds[f"{key}_truth"]    = (("anchor_time", "lead_h", "lat", "lon"), plev_truth[key])
        out_ds[f"{key}_baseline"] = (("anchor_time", "lead_h", "lat", "lon"), plev_base[key])
        out_ds[f"{key}_v18"]      = (("anchor_time", "lead_h", "lat", "lon"), plev_full[key])
