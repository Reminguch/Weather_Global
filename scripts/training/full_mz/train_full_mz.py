...
# This script lives at scripts/training/full_mz/train_full_mz.py, so the repo
# root is 3 parents up (not 2 as in the original script).
ROOT = Path(__file__).resolve().parents[3]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

# Put scripts/training/ on sys.path so we can reuse train_graphcast.
...
# FullMZ: operate on the full set of GraphCast target variables, not just the
# four "resolved" fields. Feature dim grows from 40 to ~83 (at 13 pressure
# levels). Surface fields and upper-air fields are distinguished by the
# PRESSURE_LEVEL_VARS set below.
RESOLVED_VARIABLES = (
...
# default to 1.0. The surface variables (MSLP, 10m wind, precipitation) are
# downweighted; 2m_temperature keeps weight 1.0. Upper-air variables
# (geopotential, temperature, winds, w, specific humidity) all keep 1.0.
