"""v18 baseline-stream residual rollout eval: per-lead-time lat-weighted RMSE.
...
Single input stream (= baseline self-rollout). At each AR step:
  ...
  next current_inputs = shift(current_inputs, baseline_pred_k)   # baseline only

This matches train_mz_v18 (baseline-stream residual) and avoids the
v17 input/target trajectory mismatch.
"""
    ...
    def two_stream_rollout(rng, inputs, targets_template, forcings, K, debug=False):
        ...
        for k in range(K):
            ...
            if k < K - 1:
                ...
                current_inp = _shift_inputs_with_state_eval(current_inp, bp, next_frc)
