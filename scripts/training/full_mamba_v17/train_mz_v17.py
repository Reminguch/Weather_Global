"""v17 = v16 architecture + TWO-STREAM AR (Exp 1 / Two-stream A).

Same v9 GCResidualWithZeroHead model. Same bptt-chunk training schedule.
The single architectural change vs v16: within a chunk of `bptt_steps`
anchors, the AR feedback path is split into TWO independent streams:

  baseline_inputs: GraphCast rolls out from ITS OWN previous prediction.
    baseline_inputs_{i+1} = shift(baseline_inputs_i, baseline_pred_i)

  residual_inputs: Mamba sees the FULL corrected prediction.
    residual_inputs_{i+1} = shift(residual_inputs_i, baseline_pred_i + residual_pred_i)

  target_i = truth_i - stop_gradient(baseline_pred_i)   # stable target!

Why: in v16's single-stream design, the frozen baseline GC was fed
(baseline + residual) as input at every AR step. GC was never trained on
this distribution -> baseline_pred drifts -> the residual training target
(truth - baseline_pred) becomes a MOVING target that depends on Mamba's
own weights. Gradient signal is unstable.

In v17 two-stream, the baseline stream is purely GC's self-rollout
(deterministic, independent of Mamba params) -> target = truth - GC_rollout
is fixed -> clean residual signal -> curriculum can actually push the
residual to learn deeper AR drift correction.

K=1 (ar-tail-K=0) is BYTE-IDENTICAL to v15 v2 / v16: only the first anchor
is fed real ERA5 and both streams collapse to the same input.
...
def main():
    ...
    # v17 TWO-STREAM AR: baseline and residual streams have independent inputs.
    # baseline stream rolls its own predictions; residual stream sees corrected.
    def _one_ar_step(rp, rs, key, baseline_inputs, residual_inputs, truth_i, forcings_i):
        baseline_pred, _ = baseline_predict.apply(
            baseline_params, baseline_state, key,
            baseline_inputs, truth_i, forcings_i)
        residual_pred, new_rs = residual_predict.apply(
            rp, rs, key, residual_inputs, truth_i, forcings_i)
        # Target = truth - GC_rollout (independent of Mamba params -> stable).
        # stop_gradient is redundant since baseline_params is frozen but kept
        # for clarity.
        ...
        (loss_da, _), _ = residual_loss.apply(
            rp, rs, key, residual_inputs, target_da, forcings_i, True)
    ...
    def _make_train_step(ar_start_static: int):
        ...
        def train_step(residual_params, residual_state, opt_state, keys,
            ...
            def f(rp):
                rs = residual_state
                losses = []
                # v17 TWO-STREAM: keep two parallel input streams.
                # In the truth zone they're identical (real ERA5). They diverge
                # only once we enter the AR-tail zone.
                baseline_inputs = inputs_truth_list[0]
                residual_inputs = inputs_truth_list[0]
                for i in range(bptt):
                    baseline_pred, residual_pred, loss_i, rs = _one_ar_step_ckpt(
                        rp, rs, keys[i],
                        baseline_inputs, residual_inputs,
                        truths_list[i], forcings_list[i])
                    losses.append(loss_i)
                    if i < bptt - 1:
                        next_i = i + 1
                        if next_i < ar_start_static:
                            # Next anchor still in truth zone -> use real ERA5 inputs
                            baseline_inputs = inputs_truth_list[next_i]
                            residual_inputs = inputs_truth_list[next_i]
                        else:
                            # AR zone:
                            #   baseline stream <- GC rolls itself (baseline_pred only)
                            #   residual stream <- corrected (baseline + residual)
                            corrected_pred = jax.tree_util.tree_map(
                                lambda b, r: b + r, baseline_pred, residual_pred)
                            baseline_inputs = _shift_inputs_with_state(
                                baseline_inputs, baseline_pred,
                                forcings_list[next_i])
                            residual_inputs = _shift_inputs_with_state(
                                residual_inputs, corrected_pred,
                                forcings_list[next_i])
