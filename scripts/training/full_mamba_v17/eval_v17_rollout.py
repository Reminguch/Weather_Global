"""v17 two-stream AR rollout eval: per-lead-time lat-weighted RMSE.

Unrolls TWO independent input streams in parallel (matching v17 training):

  baseline stream:   GC sees its own previous prediction
  residual stream:   Mamba sees the previous CORRECTED prediction (baseline + residual)

At each lead time k we emit:
  baseline_pred_k = GC(baseline_stream)
  full_pred_k     = baseline_pred_k + residual_pred_k
                    where residual_pred_k = Mamba(residual_stream)

This matches what train_mz_v17 sees during AR training and avoids the
single-stream v16 eval/train mismatch.
"""
...
def main():
    ...
    def two_stream_rollout(rng, inputs, targets_template, forcings, K, debug=False):
        """Roll out K steps with two parallel input streams.
        Returns (baseline_traj [time=K], full_traj [time=K]).

        If debug=True, prints per-step ||baseline_inputs - residual_inputs||
        so you can confirm two-stream is actually live (t=0 should be 0,
        t>=1 should be nonzero and growing in AR zone).
        """
        baseline_inp = inputs
        residual_inp = inputs
        baseline_chunks = []
        full_chunks = []
        rs = residual_state  # Mamba state propagated within this rollout
        if debug:
            print("[two_stream_rollout debug] tracking stream divergence:")
        for k in range(K):
            if debug:
                _divs = []
                for v in baseline_inp.data_vars:
                    if v in residual_inp.data_vars:
                        a = np.asarray(baseline_inp[v].values)
                        b = np.asarray(residual_inp[v].values)
                        d = float(np.max(np.abs(a - b)))
                        if d > 0:
                            _divs.append(f"{v}={d:.3e}")
                msg = ", ".join(_divs) if _divs else "(zero, streams identical)"
                print(f"  k={k} input_diff_max_abs: {msg}")
            ...
            bp = _baseline_step(baseline_params, baseline_state, rng_b,
                                baseline_inp, tgt_k, frc_k)
            rp, rs = _residual_step(residual_params, rs, rng_r,
                                    residual_inp, tgt_k, frc_k)
            corrected = jax.tree_util.tree_map(lambda b, r: b + r, bp, rp)
            baseline_chunks.append(bp)
            full_chunks.append(corrected)
            if k < K - 1:
                next_frc = forcings.isel(time=slice(k + 1, k + 2))
                baseline_inp = _shift_inputs_with_state_eval(baseline_inp, bp, next_frc)
                residual_inp = _shift_inputs_with_state_eval(residual_inp, corrected, next_frc)
