#!/usr/bin/env python3
"""v4.1 trainer: hybrid stateful-cycling + K-step closed-loop residual Mamba.

Differences from v4:
  * v4 trained K=1 single-step (every position truth-fed inside the chunk).
  * v4.1 keeps the v4 stateful-cycling backbone (TBPTT, lanes, num_lanes),
    BUT inside each chunk introduces K-step closed-loop rollout:
      - tf_mask = [1,0,...,0,  1,0,...,0, ...]  (anchor every K positions)
      - Anchor positions: residual head sees TRUE prev-cycle residual.
      - Non-anchor (intra-anchor) positions: residual head sees its OWN
        previously predicted residual (model self-feed).
    This addresses v4's "lead-3 cross-over to negative" by exposing the
    residual head to the model's own self-fed residual distribution at
    training time (matches deployment AR regime).
  * Lead-weighted loss: weights = (1 + lead_idx) ** loss_lead_weight_power
    applied to per-lead losses inside each anchor's K rollout (so deeper
    leads get higher gradient weight, similar to v3 group_loss).

Design (unchanged from v4):
...
class RunConfigV41:
    ...
    target_steps_closed_loop: int  # v4.1: K for in-chunk closed-loop rollout
    loss_lead_weight_power: float  # v4.1: lead-weight ** power for loss
    # Optimizer
    max_steps: int
    eval_every: int
...
def parse_args() -> RunConfigV4:
    ...
    p.add_argument("--baseline-batch-chunk", type=int, default=8,
                   help="Mini-batch size for batched baseline (GraphCast) forward. "
                        ...
                        "for fewer calls.")
    p.add_argument("--target-steps-closed-loop", type=int, default=4,
                   help="v4.1: K for in-chunk closed-loop rollout. tf_mask gets "
                        "1 every K positions (anchor truth-feed) and 0 in between "
                        "(model self-feed). Must divide --bptt-steps. Set to 1 "
                        "to recover v4 behaviour (every position truth-fed).")
    p.add_argument("--loss-lead-weight-power", type=float, default=1.0,
                   help="v4.1: per-lead loss weight = (1 + lead_idx) ** power. "
                        "Set to 0 for uniform across leads.")
    # Optimizer
    ...
    if args.bptt_steps % args.target_steps_closed_loop != 0:
        raise ValueError(
            f"--bptt-steps={args.bptt_steps} must be divisible by "
            f"--target-steps-closed-loop={args.target_steps_closed_loop}.")
    ...
    return RunConfigV41(
        ...
        target_steps_closed_loop=args.target_steps_closed_loop,
...
def main() -> None:
    ...
    # v4.1: Lead-weighted loss multiplier (T,). Position t falls at lead
    # (t mod K) within its anchor; weight = (1 + lead_idx) ** power.
    # Normalised so mean(weight) == 1 (preserves overall loss scale).
    K_cl = cfg.target_steps_closed_loop
    _lead_idx_per_pos = jnp.array(
        [(t % K_cl) for t in range(cfg.bptt_steps)], dtype=jnp.float32)
    _lead_weight_T = (1.0 + _lead_idx_per_pos) ** cfg.loss_lead_weight_power
    _lead_weight_T = _lead_weight_T / jnp.mean(_lead_weight_T)
    print(f"[v4.1] K_closed_loop={K_cl}  loss_lead_weight (T={cfg.bptt_steps}) = "
          f"{[float(x) for x in _lead_weight_T]}")
    # Reshape for broadcast over [T, B, lat, lon, F]
    lead_weight_TBHWF = _lead_weight_T[:, None, None, None, None]
    ...
    def _finalise_loss(pred_residual, baseline_next, truth_next):
        ...
        # v4.1: scale each T position by its lead weight (deeper leads get
        # higher gradient weight)
        weighted_sq_state = jnp.square(norm_diff) * w_lat * w_chan * lead_weight_TBHWF
        weighted_sq_res = jnp.square(norm_res_diff) * w_lat * w_chan * lead_weight_TBHWF
        state_loss = jnp.mean(weighted_sq_state)
        residual_loss = jnp.mean(weighted_sq_res)
        weighted_sq_group = jnp.square(norm_diff) * w_lat * lead_weight_TBHWF
        def _gm(arr, idx):
            if idx is None:
    ...
    def chunk_objective(seq_inputs, baseline_next, truth_next, h_in, is_training):
        ...
        T = cs_n.shape[0]
        # v4.1: tf_mask = [1, 0, ..., 0,  1, 0, ..., 0, ...] anchor every K
        # positions, intra-anchor positions self-feed. K_cl=1 recovers v4.
        K = cfg.target_steps_closed_loop
        tf_mask = jnp.array(
            [(1.0 if (t % K) == 0 else 0.0) for t in range(T)], dtype=jnp.float32)
