...
def parse_args() -> RunConfig:
    ...
    # Allow segment_steps=1 for sliding-window AR training (standard AR paradigm,
    # one anchor per sample). Original target_rollout setup required >=4 to
    # have meaningful packing; we relaxed this for the slide variant.
    if args.segment_steps < 1:
        raise ValueError("--segment-steps must be >= 1")
    if args.max_steps <= 0:
...
def _segment_to_tensors(
    ...
    *,
    ...
    if len(segment_indices) < 1:
        raise ValueError("Segment must contain at least 1 step.")
    # For sliding-window AR (segment_steps=1), there is only one anchor per
    # segment, so the np.diff check would be on an empty array and is a no-op.
    if len(segment_indices) >= 2 and not np.all(np.diff(segment_indices) == 1):
        raise ValueError(
