"""v16 = v13 architecture (v9 GCResidualWithZeroHead) + closed-loop AR
within each BPTT chunk.

Difference from v13/v15 v2: within a chunk of `bptt_steps` anchors, the
first anchor receives real ERA5 inputs; for anchors 2..bptt the input
window is shifted forward with the model's OWN previous corrected
prediction (Option-2 state feedback). Loss is summed across all bptt
anchors. Mamba state propagates through the chunk; gradient flows back
through the AR rollout, including the self-feed path.

This makes the train-time input distribution match the eval-time AR
closed-loop distribution -- the same training setup v3 uses, but at
the BPTT level rather than intra-anchor.

Precomputed residual targets are NOT usable (baseline_pred at steps 2..K
depends on drifted inputs); the baseline GC1 must be forwarded live each
step. Expected slowdown ~ 2x per BPTT step compared to v13.
...
def main():
    ...
    def _shift_inputs_with_pred(prev_inputs, baseline_pred, residual_pred, forcings_next):
        """Build next anchor's input window from current prediction.
        prev_inputs has time=input_steps (=2 for 12h window).
        full_pred = baseline_pred + residual_pred (next-step state, time=1).
        Shift: drop time=0 slot, shift time=1 -> time=0, append full_pred as time=1.
        For forcings we use the actual next-anchor's forcings."""
        full_pred = jax.tree_util.tree_map(lambda b, r: b + r, baseline_pred, residual_pred)
        # Merge forcings into the next frame so input set is complete
        next_frame = xr.merge([full_pred, forcings_next])
        # Drop datetime coords if any (matching rollout.chunked_prediction_generator)
        if "datetime" in next_frame.coords:
            next_frame = next_frame.drop_vars("datetime")
        # Concat along time then take tail of input_steps
        keys_in_next = [k for k in next_frame.data_vars if k in prev_inputs.data_vars]
        next_inputs_part = next_frame[keys_in_next]
        # Align time coordinate to prev_inputs convention
    ...
    # Wrap one AR-step worth of compute in jax.checkpoint so mesh-level
    # activations are recomputed during backward pass; only the grid
    # outputs (and Mamba state) are kept across steps. This trades
    # ~2x compute for ~10x activation memory.
            ...
            def f(rp):
                ...
                for i in range(bptt):
                    baseline_pred, residual_pred, loss_i, rs = _one_ar_step_ckpt(
                        rp, rs, keys[i], current_inputs, truths_list[i], forcings_list[i])
                    ...
                    if i < bptt - 1:
                        ...
                        if next_i < ar_start_static:
                            ...
                            # AR zone -> feed back model's own corrected pred
                            current_inputs = _shift_inputs_with_pred(
                                current_inputs, baseline_pred, residual_pred,
