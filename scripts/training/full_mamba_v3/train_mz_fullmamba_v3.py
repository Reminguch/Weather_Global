...
# Atmospheric-only set: drops the 5 surface variables (MSLP, 2m_T, 10m_u/v,
# precip). Per-variable eval shows surface variables degrade as K increases
# while upper-air (Z/T/q/u/v/w) keeps improving with longer rollouts. Training
# residual on the atmospheric subset removes the multi-task loss dilution from
# surface gradients and isolates the K-curriculum benefit on memory-using vars.
RESOLVED_VARIABLES_ATMOS = (
    "temperature",
    "geopotential",
    "u_component_of_wind",
    "v_component_of_wind",
...
class RunConfig:
    ...
    atmos_only: bool           # if True, override to the 6-variable atmospheric
                               # subset (Z, T, q, u, v, w; 78 channels). Used to
                               # isolate memory-using variables from surface
                               # variables that degrade under long K rollouts.
    # --- v2 modifications (Mod G / Mod A / Mod C) ---------------------------
    use_specialist_heads: bool # Mod G: split residual head into upper-air and
                               # surface heads so surface gradients don't pull
                               # on upper-air capacity at K↑.
    anchor_as_batch: bool      # Mod A: feed S anchors as the batch dim and K as
                               # the Mamba time dim, so SSM time axis aligns to
                               # monotonic physical time within each anchor.
                               # Default False = legacy S*K flattening.
    anchor_as_batch_min_k: int # Patch 1: only apply anchor-as-batch when
                               # target_steps >= this value. Default 2 keeps
                               # K=1 in legacy stateful-cycling layout (which
                               # was a net positive for K=1 v1) while applying
                               # the clean anchor batching at K>=2 where the
                               # rollout-time-axis fix is meaningful.
    eager_load_data: bool      # v3: opt-in eager xr.Dataset.load() at startup
                               # so per-step batch build is numpy-only (no
                               # zarr roundtrip). ~15-21% step speedup but
                               # ~63 GB host RAM for 2 years 1° 11-var, can
                               # OOM with anchor-as-batch + JAX buffers on
                               # 160GB nodes. Default False for safety.
    loss_lead_weight_power: float  # Mod C: per-step weighting alpha_s = (s+1)^p
                               # / mean. p=0 -> uniform (legacy). p=1 linear,
                               # p=1.5 / 2 emphasises long horizons.
    # --- v3+ three-group loss (upper / MSLP / small-surface) ---------------
    # The shared SSM body's gradient is sum over loss components. Surface
    # variables (10m wind, precip, 2mT) are noisy / displacement-dominated
    # and can swamp the long-lead memory signal carried by upper-air vars
    # (Z/T/q) and MSLP. Splitting the loss into three groups with
    # independently tunable weights lets the body's gradient prioritise the
    # memory-friendly variables without zeroing surface entirely.
    # Group composition (fixed): upper = Z/T/q/u/v/w; mslp = MSLP only;
    # small_surface = 2mT/10m_u/10m_v/total_precip.
    use_group_loss: bool       # if False (default), keep legacy single-mean
                               # weighted-MSE loss; if True use the three-
                               # group sum below.
    upper_loss_weight: float   # weight on upper-air group mean
    mslp_loss_weight: float    # weight on MSLP group mean
    small_surface_loss_weight: float  # weight on 2mT/10m_uv/precip group mean
...
def parse_args() -> RunConfig:
    ...
    parser.add_argument("--full-variables", action="store_true", default=False,
                        help="Use the full 11-variable GraphCast target set (F=83) "
                             ...
                             "can be enabled.")
    parser.add_argument("--atmos-only", action="store_true", default=False,
                        help="Override variable set to the 6-variable atmospheric "
                             "subset (T/Z/u/v/w/q at 13 pressure levels; F=78). "
                             "Drops 5 surface variables (MSLP, 2m_T, 10m_u/v, "
                             "precip) which degrade under K-curriculum. Mutually "
                             "exclusive with --full-variables (atmos_only wins).")
    # --- v2 modifications -----------------------------------------------------
    parser.add_argument("--specialist-heads", action="store_true", default=False,
                        help="Mod G: split residual head into upper-air and "
                             "surface heads (independent Linear projections "
                             "from the shared SSM body). Requires --full-variables "
                             "(needs both groups). Default off for v1 parity.")
    parser.add_argument("--anchor-as-batch", action="store_true", default=False,
                        help="Mod A: feed S anchors as batch and K as Mamba time, "
                             "so SSM hidden state propagates over a monotonic "
                             "physical time axis within each anchor and resets "
                             "between anchors. Default off for v1 parity.")
    parser.add_argument("--anchor-as-batch-min-k", type=int, default=2,
                        help="Patch 1: --anchor-as-batch only takes effect when "
                             "target_steps >= this value. Default 2 keeps K=1 in "
                             "legacy layout (preserves v1 stateful cycling that "
                             "out-performs single-step independent anchors at "
                             "K=1) while applying anchor-as-batch at K>=2 where "
                             "the rollout-time-axis fix matters. Set to 1 to "
                             "force anchor-as-batch even at K=1.")
    parser.add_argument("--eager-load-data", action="store_true", default=False,
                        help="v3: eager-load train_ds + eval_ds into RAM at "
                             "startup via xr.Dataset.load(). ~15-21%% step "
                             "speedup but ~63 GB RAM cost for 2-year 1° dataset. "
                             "Default OFF (memory-safe lazy zarr path). Turn on "
                             "only for short runs or large-mem nodes (>=240GB).")
    parser.add_argument("--loss-lead-weight-power", type=float, default=0.0,
                        help="Mod C: per-step loss weight power p. Weight at "
                             "lead s (1-indexed) is (s+1)^p / mean over K. "
                             "p=0 uniform (default), p=1 linear, p=1.5/2 emphasis "
                             "on long horizons.")
    parser.add_argument("--use-group-loss", action="store_true", default=False,
                        help="Three-group loss: split feature channels into "
                             "upper-air (Z/T/q/u/v/w), MSLP, and small-surface "
                             "(2mT/10m wind/precip), compute the (lat * channel "
                             "* lead)-weighted MSE inside each group, then "
                             "combine as a weighted sum with --upper-loss-weight, "
                             "--mslp-loss-weight, --small-surface-loss-weight. "
                             "Default OFF preserves legacy single-mean loss.")
    parser.add_argument("--upper-loss-weight", type=float, default=1.0,
                        help="Weight on upper-air group mean when --use-group-loss.")
    parser.add_argument("--mslp-loss-weight", type=float, default=1.0,
                        help="Weight on MSLP group mean when --use-group-loss. "
                             "MSLP is large-scale pressure and is the primary "
                             "memory-benefiting surface variable, so it gets "
                             "its own weight rather than being lumped with the "
                             "noisy small-surface group.")
    parser.add_argument("--small-surface-loss-weight", type=float, default=0.1,
                        help="Weight on small-surface group mean when "
                             "--use-group-loss. Default 0.1 attenuates the "
                             "noisy 2mT/10m_wind/precip gradients on the "
                             "shared Mamba body, keeping memory-friendly "
                             "variables from being swamped while still "
                             "training the surface heads.")
    ...
    # Guard 1: --specialist-heads is wired only into the (meshed + full_mamba +
    # full_variables) factory path. Reject configs where the flag would be
    # silently ignored or produce a degenerate model.
    if args.specialist_heads:
        if not args.full_mamba:
            raise ValueError(
                "--specialist-heads requires --full-mamba (specialist heads "
                "are only wired into MZResidualFullMambaMeshed)."
            )
        if not args.meshed:
            raise ValueError(
                "--specialist-heads requires --meshed (per-grid factory has "
                "no head split)."
            )
        if not args.full_variables:
            raise ValueError(
                "--specialist-heads requires --full-variables (needs both "
                "the upper-air and surface channel groups; the 4-variable "
                "default has no surface variables to specialise)."
            )
        if args.atmos_only:
            raise ValueError(
                "--specialist-heads is incompatible with --atmos-only "
                "(surface_dim would be 0)."
            )

    if args.anchor_as_batch_min_k < 1:
        raise ValueError("--anchor-as-batch-min-k must be >= 1.")

    # Three-group loss only makes physical sense when the full variable set is
    # active (groups are upper / MSLP / small-surface). The 4-var legacy set
    # has only MSLP + Z/u/v so the small-surface group would be empty and the
    # MSLP group would be a single channel — possible to run, but the group
    # weighting story stops being useful. Reject explicitly.
    if args.use_group_loss:
        if not args.full_variables:
            raise ValueError(
                "--use-group-loss requires --full-variables: groups are "
                "upper / MSLP / small-surface and need the 11-variable set."
            )
        if args.atmos_only:
            raise ValueError(
                "--use-group-loss is incompatible with --atmos-only "
                "(MSLP and small-surface groups would both be empty)."
            )
        if (args.upper_loss_weight < 0 or args.mslp_loss_weight < 0
                or args.small_surface_loss_weight < 0):
            raise ValueError("group loss weights must be >= 0")
        weight_sum = (args.upper_loss_weight + args.mslp_loss_weight
                      + args.small_surface_loss_weight)
        if weight_sum <= 0:
            raise ValueError(
                "group loss weights must sum to > 0 (got "
                f"upper={args.upper_loss_weight}, "
                f"mslp={args.mslp_loss_weight}, "
                f"small_surface={args.small_surface_loss_weight})."
            )
    ...
    return RunConfig(
        ...
        use_specialist_heads=args.specialist_heads,
        anchor_as_batch=args.anchor_as_batch,
        anchor_as_batch_min_k=args.anchor_as_batch_min_k,
        eager_load_data=args.eager_load_data,
        loss_lead_weight_power=args.loss_lead_weight_power,
...
def _segment_to_tensors(
    ...
    *,
    ...
    if anchor_as_batch:
        # Mod A: each anchor is its own batch sample, K is the Mamba time axis.
        # Layout: [T=K, B=S, lat, lon, F] (transpose [S, K, ...] -> [K, S, ...]).
        # The SSM hidden state becomes per-(anchor, mesh_node) and resets
        # automatically between segments. tf_mask is [K] = [1, 0, 0, ..., 0],
        # broadcast across all S anchors uniformly.
        seg_axes = (1, 0) + tuple(range(2, current_state_SK.ndim))
        current_state_t = jnp.transpose(current_state_SK, seg_axes)  # [K, S, lat, lon, F]
        baseline_t = jnp.transpose(baseline_SK, seg_axes)
        truth_t = jnp.transpose(truth_SK, seg_axes)
        teacher_prev_t = jnp.transpose(teacher_prev_residual_SK, seg_axes)
        tf_mask = jnp.zeros((K,), dtype=jnp.float32).at[0].set(1.0)
        seq_inputs = jnp.concatenate([current_state_t, teacher_prev_t], axis=-1)
        return seq_inputs, baseline_t, truth_t, tf_mask

    # Legacy path (S*K flatten). Used when --anchor-as-batch is not set.
...
def main() -> None:
    ...
    if cfg.atmos_only:
        RESOLVED_VARIABLES = RESOLVED_VARIABLES_ATMOS
    elif cfg.full_variables:
        RESOLVED_VARIABLES = RESOLVED_VARIABLES_FULL
    else:
        RESOLVED_VARIABLES = RESOLVED_VARIABLES_MIN
    print(
        f"[vars] atmos_only={cfg.atmos_only}  full_variables={cfg.full_variables}  "
        f"n_vars={len(RESOLVED_VARIABLES)}  list={RESOLVED_VARIABLES}"
    )

    # Patch 1: resolve effective anchor-as-batch decision once.
    effective_anchor_as_batch = (
        cfg.anchor_as_batch and cfg.target_steps >= cfg.anchor_as_batch_min_k
    )
    print(
        f"[layout] requested_anchor_as_batch={cfg.anchor_as_batch}  "
        f"min_k={cfg.anchor_as_batch_min_k}  target_steps={cfg.target_steps}  "
    ...
    # v3 fast-data: eager .load() materialises the prepared datasets into RAM
    # so per-step ``build_batch_from_indices`` doesn't hit zarr lazy loads.
    # Trade-off: ~15-21% step speedup, but ~63 GB host RAM for 2 years 1° 11-var
    # dataset. Combined with anchor-as-batch's larger SSM hidden batch + JAX
    # buffers, this can OOM on 160GB SLURM nodes (observed at K=2 step 5800
    # with 160GB allocation).
    # The flag --eager-load-data (default OFF) lets you opt in only on
    # large-mem nodes / short runs. Default behaviour is the same as v2: lazy
    # zarr-backed datasets, ~13s/step but no OOM risk.
    if getattr(cfg, "eager_load_data", False):
        import time as _t_load
        _t0 = _t_load.time()
        print(f"[data] eager_load_data=True: loading train_ds + eval_ds into RAM ...")
        train_ds = train_ds.load()
        eval_ds = eval_ds.load()
        print(f"[data] eager load done in {_t_load.time() - _t0:.1f}s. "
              f"Per-step batch build is now numpy-only (no zarr roundtrip).")
    else:
        print(f"[data] eager_load_data=False (default, memory-safe). "
              f"Per-step batch build hits zarr lazy loads, ~13s/step.")
    ...
    # Three-group channel index masks for --use-group-loss. Built always so the
    # corresponding group-mean diagnostics can be logged even when the group
    # loss is OFF (helps decompose the legacy total later from the same logs).
    _SMALL_SURFACE_NAMES = {
    ...
    for name in feature_order:
        ...
        if name == "mean_sea_level_pressure":
            mslp_idx_list.extend(idxs)
        elif name in _SMALL_SURFACE_NAMES:
            small_idx_list.extend(idxs)
        else:
            upper_idx_list.extend(idxs)
    if not upper_idx_list:
        # 4-var legacy set has no upper-air subset; fall back to all-channel
        # weights = 1.0 so use_group_loss collapses to plain mean.
        upper_idx_list = list(range(feature_dim))
    upper_idx_jnp = jnp.asarray(upper_idx_list, dtype=jnp.int32)
    ...
    print(
        f"  loss groups: upper={len(upper_idx_list)} ch  "
        f"mslp={len(mslp_idx_list)} ch  small_surface={len(small_idx_list)} ch  "
        f"(use_group_loss={cfg.use_group_loss})"
    )
    if cfg.use_group_loss:
        print(
            f"  group weights: upper={cfg.upper_loss_weight}  "
            f"mslp={cfg.mslp_loss_weight}  small_surface={cfg.small_surface_loss_weight}"
        )
    loss_norm_f = (
    ...
    if cfg.meshed:
        ...
        if cfg.full_mamba:
            # Specialist heads (Mod G): build channel index lists from the
            # active variable set. Surface = (2m_T, MSLP, 10m_u, 10m_v, precip),
            # everything else (the 6 pressure-level vars × 13 levels) is upper.
            if cfg.use_specialist_heads:
                ...
                # Guard 2: indices must be a clean permutation of 0..F-1 with
                # no overlap and no gap. Cheap to assert and prevents silent
                # mis-routing in the scatter/gather decoder.
                up_set = set(upper_idx)
                sf_set = set(surface_idx)
                if up_set & sf_set:
                    raise ValueError(
                        f"specialist heads: upper/surface indices overlap: "
                        f"{sorted(up_set & sf_set)}"
                    )
                all_idx = sorted(upper_idx + surface_idx)
                if all_idx != list(range(feature_dim)):
                    raise ValueError(
                        f"specialist heads: indices do not cover 0..F-1 "
                        f"(F={feature_dim}, n={len(all_idx)}, head={all_idx[:8]}...)"
                    )
                upper_idx_t = tuple(upper_idx)
                surface_idx_t = tuple(surface_idx)
                print(
                    f"[specialist_heads] upper_dim={len(upper_idx_t)} "
                    f"surface_dim={len(surface_idx_t)} "
                    f"total={len(upper_idx_t) + len(surface_idx_t)}"
                )
            else:
                upper_idx_t = ()
                surface_idx_t = ()
    ...
    # Mod C: per-lead-time weight alpha_s = (s+1)^p / mean over K. p=0 uniform.
    # Weight axis is the Mamba time axis (T): in legacy mode T = S*K (anchor
    # boundaries embedded), in anchor_as_batch T = K (clean lead time). The
    # decision uses ``effective_anchor_as_batch`` so K=1 fallback gets the
    # correct legacy weighting semantics.
    def _make_lead_weights(T_dim: int) -> jax.Array:
        p = cfg.loss_lead_weight_power
        if p == 0.0:
            return jnp.ones((T_dim,), dtype=jnp.float32)
        if effective_anchor_as_batch:
            steps = jnp.arange(1, T_dim + 1, dtype=jnp.float32)
        else:
            # Legacy S*K layout: lead within each anchor is (T_idx mod K) + 1.
            K = cfg.target_steps
            steps = (jnp.arange(T_dim, dtype=jnp.float32) % K) + 1.0
        w = jnp.power(steps, p)
        return w / jnp.mean(w)

    def _finalise_loss(pred_residual, baseline_next, truth_next):
        """Loss with two parallel weightings:

          * legacy state_loss / residual_loss: GraphCast-style per-channel
            (lat * level * per-var) weighted normalised MSE; used as the
            optimized objective when ``cfg.use_group_loss`` is False so
            existing chains reproduce byte-for-byte.

          * group losses: lat * lead-weighted normalised MSE WITHOUT the
            per-variable ``w_chan`` weighting. The old GraphCast w_chan would
            silently re-multiply MSLP / 10m wind / precip by 0.1, which means
            even when --mslp-loss-weight=1.0 the effective MSLP contribution
            would still be 0.1. We strip w_chan from the group computation so
            ``cfg.mslp_loss_weight`` / etc. is the *full* per-group weight.

        When ``cfg.use_group_loss`` the optimized total is the (lambda-)
        normalised weighted sum of the three group means, so changing the
        relative weights does not also rescale the overall loss magnitude
        (keeps LR transferable from legacy runs).
        """
        ...
        # Mod C: lead-time weight on the time axis (axis 0).
        T_dim = corrected.shape[0]
        w_lead = _make_lead_weights(T_dim)[:, None, None, None, None]

        # Legacy GraphCast-style normalised MSE (with per-variable w_chan).
        weighted_sq_state_legacy = jnp.square(norm_diff) * w_lat * w_chan * w_lead
        weighted_sq_res_legacy = jnp.square(norm_res_diff) * w_lat * w_chan * w_lead
        state_loss = jnp.mean(weighted_sq_state_legacy)
        residual_loss = jnp.mean(weighted_sq_res_legacy)

        # Group loss: deliberately omit w_chan so the group weights are the
        # final per-group weighting. Each variable inside a group is treated
        # equally (subject to the level/mean(level) part baked into w_chan,
        # which we drop here too — group weighting is meant to be a clean
        # variable-set re-weighting, not a level-weighting tweak).
        weighted_sq_state_group = jnp.square(norm_diff) * w_lat * w_lead

        def _group_mean(arr, idx):
            if idx is None:
                return jnp.asarray(0.0, dtype=arr.dtype)
            return jnp.mean(jnp.take(arr, idx, axis=-1))

        loss_upper = _group_mean(weighted_sq_state_group, upper_idx_jnp)
        loss_mslp = _group_mean(weighted_sq_state_group, mslp_idx_jnp)
        loss_small = _group_mean(weighted_sq_state_group, small_idx_jnp)

        if cfg.use_group_loss:
            den = (
                cfg.upper_loss_weight
                + cfg.mslp_loss_weight
                + cfg.small_surface_loss_weight
            )
            # den > 0 enforced in parse_args.
            total_loss = (
                cfg.upper_loss_weight * loss_upper
                + cfg.mslp_loss_weight * loss_mslp
                + cfg.small_surface_loss_weight * loss_small
            ) / den
        else:
    ...
    if cfg.resume_from is not None:
        ...
        # Guard 3: resuming a v1 single-residual_head ckpt into a v2
        # specialist-heads model is an unfair head reset. Detect this and
        # require the user to acknowledge by passing --allow-partial-resume,
        # since the old residual_head/{w,b} have no destination and
        # upper_head/surface_head will fresh-init to zero.
        if cfg.use_specialist_heads:
            ckpt_has_single_head = any(
                "residual_head" in k for k in loaded_params.keys()
            )
            ckpt_has_split_head = any(
                ("upper_head" in k or "surface_head" in k)
                for k in loaded_params.keys()
            )
            if ckpt_has_single_head and not ckpt_has_split_head:
                if not cfg.allow_partial_resume:
                    raise ValueError(
                        "Resume source has v1 single residual_head, but the "
                        "current model uses --specialist-heads. The old head "
                        "will not transfer (its slice would be lost) and the "
                        "new upper_head/surface_head will fresh zero-init. "
                        "Pass --allow-partial-resume to acknowledge, or "
                        "convert the ckpt by slicing residual_head into "
                        "upper_head/surface_head columns by upper/surface "
                        "channel index."
                    )
                print(
                    "[resume_warn] v1 single-head ckpt → v2 specialist-heads. "
                    "Body params transfer; upper_head/surface_head fresh "
                    "zero-init. Early steps will look like training from a "
                    "post-body-only init (corrected ≈ baseline)."
                )
    ...
    for step in range(start_step, cfg.max_steps + 1):
        ...
        (mem_params, opt_state, loss, state_loss, residual_loss,
         loss_upper, loss_mslp, loss_small_surface, grad_norm) = train_step(
        ...
        train_log.append(
            {
                ...
                "loss_small_surface": float(loss_small_surface),
        ...
        if step == 1 or step % 10 == 0:
            print(
                ...
                f"state {float(state_loss):.6f} residual {float(residual_loss):.6f} "
                f"upper {float(loss_upper):.5f} mslp {float(loss_mslp):.5f} small {float(loss_small_surface):.5f} "
