"""Diagnostic: save baseline_pred / full_pred / truth fields for tp_6h
to inspect why MAE and RMSE disagree (skewed error distribution).
Runs K=6 AR rollout on N val anchors, saves a .npz with shape
(N, K, lat, lon) for each of {truth, baseline, full}.
"""
...
def parse_args():
    ...
    p.add_argument("--level", type=int, default=None,
                   help="If variable has a level dim, slice to this pressure level (hPa).")
...
def main():
    ...
    print(f"[diag] loaded {ckpt_path.name}")
    ...
    chosen = sorted(rng_np.choice(val_indices, size=cfg.n_anchors, replace=False).tolist())

    K = cfg.target_steps
    truth_all = np.zeros((cfg.n_anchors, K, eval_ds.sizes["lat"], eval_ds.sizes["lon"]), dtype=np.float32)
    base_all  = np.zeros_like(truth_all)
    full_all  = np.zeros_like(truth_all)
    ...
    for i, idx in enumerate(chosen):
        ...
        def _extract(ds):
            arr = ds[cfg.var].astype("float32")
            if "level" in arr.dims:
                if cfg.level is None:
                    raise ValueError(f"--level required for var {cfg.var} which has level dim")
                arr = arr.sel(level=cfg.level)
            # Reduce singleton batch, then ensure canonical (time, lat, lon)
            if "batch" in arr.dims:
                arr = arr.isel(batch=0)
            arr = arr.transpose("time", "lat", "lon")
            return arr.values
        truth_all[i] = _extract(tgt)
        base_all[i]  = _extract(bp)
        full_all[i]  = _extract(fp)
        print(f"[diag] anchor {i+1}/{cfg.n_anchors} idx={idx}")

    out = Path(cfg.out_npz)
    out.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(out,
                        truth=truth_all, baseline=base_all, full=full_all,
                        lat=eval_ds.lat.values, lon=eval_ds.lon.values,
                        chosen_idx=np.array(chosen),
                        var=cfg.var)
    print(f"[diag] wrote {out} (shape {truth_all.shape})")
