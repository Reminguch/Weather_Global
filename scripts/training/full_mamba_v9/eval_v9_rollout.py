"""v9 multi-step AR rollout eval: per-lead-time lat-weighted RMSE.

Manually unrolls the residual + baseline pair via graphcast.rollout's
`chunked_prediction_generator` with `num_steps_per_chunk=1`. The full
trajectory and the baseline-only trajectory are unrolled INDEPENDENTLY
closed-loop (each feeds its own prediction back). At each lead time k
we compute lat-weighted RMSE vs ground truth.
"""
...
def main():
    ...
    def full_predictor(rng, inputs, targets_template, forcings):
        ...
        bp = _baseline_step(baseline_params, baseline_state, rng_b,
                            inputs, targets_template, forcings)
        rp, _ = _residual_step(residual_params, residual_state, rng_r,
                               inputs, targets_template, forcings)
    ...
    for s_i, idx in enumerate(chosen_idx):
        ...
        # Generate baseline and full trajectories independently (each feeds
        # its own prediction back).
        rng, rng_b_chain, rng_f_chain = jax.random.split(rng, 3)
        b_chunks = list(rollout.chunked_prediction_generator(
            baseline_predictor, rng_b_chain, inp, tgt, frc,
            num_steps_per_chunk=1))
        f_chunks = list(rollout.chunked_prediction_generator(
            full_predictor, rng_f_chain, inp, tgt, frc,
            num_steps_per_chunk=1))
        baseline_pred = xr.concat(b_chunks, dim="time")
        full_pred = xr.concat(f_chunks, dim="time")
