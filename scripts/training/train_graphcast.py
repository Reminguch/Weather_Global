...
class RunConfig:
    ...
    eval_only: bool
    trainable_part: str          # "all" | "mamba" | "graphcast"
...
def parse_args() -> RunConfig:
    ...
    parser.add_argument(
        ...
        default="all",
        help="Which params receive gradient updates. 'mamba' freezes "
             "GraphCast and only trains temporal/Mamba params (Ilya's "
             "default for residual-style training on a frozen backbone).",
    )
...
def main() -> None:
    ...
    # Inherit edge normalization from ckpt (forcing None changes encoder MLP
    # input channel count and breaks DeepMind-public-ckpt overlay).
    ...
    print(f"[debug] task_cfg.input_variables ({len(task_cfg.input_variables)}): {task_cfg.input_variables}")
    print(f"[debug] task_cfg.target_variables ({len(task_cfg.target_variables)}): {task_cfg.target_variables}")
    print(f"[debug] task_cfg.forcing_variables ({len(task_cfg.forcing_variables)}): {task_cfg.forcing_variables}")
    print(f"[debug] task_cfg.pressure_levels ({len(task_cfg.pressure_levels)}): {task_cfg.pressure_levels}")
    print(f"[debug] task_cfg.input_duration: {task_cfg.input_duration}")
    print(f"[debug] sample_inputs vars: {sorted(sample_inputs.data_vars)}")
    print(f"[debug] sample_inputs sizes: {dict(sample_inputs.sizes)}")
    ...
    if cfg.resume_step is not None:
        ...
        # Overlay DeepMind public ckpt params into the GC layers of our (GC +
        # Mamba) model. Mamba layers have no match in ckpt and stay at fresh
        # init. This is the correct behaviour when --trainable-part mamba is
        # set (frozen DeepMind GC + trainable Mamba from random).
        ...
        print(
            f"Overlaid {overlay_stats.copied} GC params from ckpt {cfg.ckpt_in}, "
            f"{overlay_stats.initialized} extra params left at fresh init (Mamba)."
        )
    ...
    # Optimizer: optionally freeze GraphCast and only train the temporal
    # Mamba parameters (mirror Ilya `--trainable-part mamba`). Identifies
    # temporal params by name containing "temporal" or "mamba".
    def _is_temporal_param(module_name: str, param_name: str) -> bool:
        path = f"{module_name}/{param_name}".lower()
        return ("temporal" in path) or ("mamba" in path)

    def _trainable_label(module_name: str, param_name: str) -> str:
        if cfg.trainable_part == "all":
            return "train"
        is_temporal = _is_temporal_param(module_name, param_name)
        if cfg.trainable_part == "mamba":
            return "train" if is_temporal else "freeze"
        if cfg.trainable_part == "graphcast":
            return "freeze" if is_temporal else "train"
        raise ValueError(f"Unsupported trainable_part={cfg.trainable_part!r}")

    if cfg.trainable_part == "all":
        opt = optax.adamw(cfg.lr, weight_decay=cfg.weight_decay)
    else:
        labels = {
            module_name: {pn: _trainable_label(module_name, pn) for pn in module_params}
        ...
        print(f"[trainable] part={cfg.trainable_part}  train={n_train}  freeze={n_freeze}")
