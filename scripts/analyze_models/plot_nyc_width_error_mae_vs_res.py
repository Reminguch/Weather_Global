...
def _extract_res_mp(path: Path) -> tuple[int, int] | None:
    m = re.match(r"nyc_width_error_res(\d+)_mp(\d+)\.csv$", path.name)
    if not m:
        return None
...
def _plot_one(
    ...
    ddf = df[(df["mp"].astype(int) == mp) & (df["lead_days"].astype(int) == lead_days)]
    if ddf.empty:
        return False
    ...
    ax.set_title(f"{title_prefix} | mp={mp}, lead={lead_days}d")
...
def main() -> None:
    ...
    csv_paths = _discover_csvs(args.input_dir)
    if not csv_paths:
