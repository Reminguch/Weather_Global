...
MEM="${RES2_DS16_K12_GROUPED_LR_EVAL_MEM:-48G}"
TIME="${RES2_DS16_K12_GROUPED_LR_EVAL_TIME:-01:30:00}"
...
RELEASE_ROOT="artifacts/checkpoints/7_years/mamba_release_all_from_frozen20k_res2_ds16_k12_gclr1e6_mlr1e5_20k"
...
for d_inner in 16 64; do
  ...
  EVAL_SPECS+=(
    "gc_mamba release_di${d_inner}_ds16_ts12_gclr1e6_mlr1e5 ${RELEASE_ROOT}/${run}_release_all20k_gclr1e6_mlr1e5/ckpt_best.npz"
  )
done
for d_inner in 16 64; do
  run="${BASE_NAME}_gc_mamba_tc2_di${d_inner}_ds16_20k_target_step12_bptt16"
  EVAL_SPECS+=(
    "gc_mamba frozen_di${d_inner}_ds16_ts12 ${FROZEN_ROOT}/${run}/ckpt_best.npz"
  )
done
EVAL_SPECS+=(
...
if [[ "${#EVAL_SPECS[@]}" -ne 6 ]]; then
  echo "Expected 6 eval specs, got ${#EVAL_SPECS[@]}" >&2
  exit 1
fi
...
for spec in "${EVAL_SPECS[@]}"; do
  ...
  if [[ ! -f "${ckpt}" ]]; then
    if [[ -n "${UPSTREAM_DEPENDENCY}" && "${label}" == release_* ]]; then
