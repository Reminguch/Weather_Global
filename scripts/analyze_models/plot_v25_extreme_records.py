"""v25 extreme-records plot: similar to /2026-05-23-v22/plots/K{K}/extreme_records_K40.png
but for the gated extreme head (v25 step3000) vs open-loop K=18 baseline at res=2.

Layout: 6 rows (leads 24h, 48h, 72h, 120h, 168h, 240h) × 6 cols
  cols: Heat RMSE | Cold RMSE | Wind RMSE | Heat bias | Cold bias | Wind bias

Each panel plots two curves vs how much the truth event exceeds the 2015-2021 record:
  - GC baseline (gray)
  - v25 (or v22 K=18) prediction (blue/green)
"""
from __future__ import annotations
import numpy as np
import xarray as xr
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import pandas as pd
from pathlib import Path

LEAD_K = [4, 8, 12, 20, 28, 40]   # 24h, 48h, 72h, 120h, 168h, 240h
T_EDGES = np.array([0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0])
W_EDGES = np.array([0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0])

DATA_DIR = Path("/scratch/gpfs/DABANIN/lm8598/Weather_Global/results/2026-6-27-v25-extreme")
RECS_RES2 = "/scratch/gpfs/DABANIN/lm8598/Weather_Global/results/0522_v22_extreme/records_climatology_2015_2021.nc"
OUT_DIR = Path("/home/lm8598/Weather_Global_experiments/results/2026-06-28-v25-extreme/plots")
OUT_DIR.mkdir(parents=True, exist_ok=True)


def bin_stats(exc, err, edges):
    centers = 0.5 * (edges[:-1] + edges[1:])
    n = len(centers); rmse = np.full(n, np.nan); bias = np.full(n, np.nan); count = np.zeros(n, int)
    for i in range(n):
        m = (exc >= edges[i]) & (exc < edges[i+1])
        if m.sum() > 0:
            e = err[m]
            rmse[i] = np.sqrt(np.mean(e**2)); bias[i] = np.mean(e); count[i] = int(m.sum())
    return count, rmse, bias, centers


def plot_one_model(preds, recs, model_name: str, color: str, out_path: Path):
    panels_template = ["Heat RMSE", "Cold RMSE", "Wind RMSE", "Heat bias", "Cold bias", "Wind bias"]
    fig, axes = plt.subplots(len(LEAD_K), 6, figsize=(22, 3.0*len(LEAD_K)))

    def _av(name, k_idx):
        return preds[name].isel(lead_h=k_idx).transpose("anchor_time", "lat", "lon").values

    for row, K_lead in enumerate(LEAD_K):
        k_idx = K_lead - 1
        lead_h = K_lead * 6
        t_tru = _av("2m_temperature_truth", k_idx)
        u_tru = _av("10m_u_component_of_wind_truth", k_idx)
        v_tru = _av("10m_v_component_of_wind_truth", k_idx)
        w_tru = np.sqrt(u_tru**2 + v_tru**2)
        t_b = _av("2m_temperature_baseline", k_idx)
        t_f = _av("2m_temperature_v18", k_idx)
        u_b = _av("10m_u_component_of_wind_baseline", k_idx)
        v_b = _av("10m_v_component_of_wind_baseline", k_idx)
        u_f = _av("10m_u_component_of_wind_v18", k_idx)
        v_f = _av("10m_v_component_of_wind_v18", k_idx)
        w_b = np.sqrt(u_b**2 + v_b**2); w_f = np.sqrt(u_f**2 + v_f**2)

        anc_times = preds.anchor_time.values
        valid_times = anc_times + np.timedelta64(lead_h, "h")
        doys = pd.DatetimeIndex(valid_times).dayofyear.values
        rmx = recs["t2m_record_max"].sel(doy=xr.DataArray(doys, dims="anchor_time")).transpose("anchor_time", "lat", "lon").values
        rmn = recs["t2m_record_min"].sel(doy=xr.DataArray(doys, dims="anchor_time")).transpose("anchor_time", "lat", "lon").values
        rwmx = recs["wind10m_record_max"].sel(doy=xr.DataArray(doys, dims="anchor_time")).transpose("anchor_time", "lat", "lon").values

        exc_h = (t_tru - rmx).ravel(); err_b_t = (t_b - t_tru).ravel(); err_f_t = (t_f - t_tru).ravel()
        exc_c = (rmn - t_tru).ravel()
        exc_w = (w_tru - rwmx).ravel(); err_b_w = (w_b - w_tru).ravel(); err_f_w = (w_f - w_tru).ravel()
        mh = exc_h > 0; mc = exc_c > 0; mw = exc_w > 0
        data_panels = [
            ("heat", exc_h[mh], err_b_t[mh], err_f_t[mh], T_EDGES),
            ("cold", exc_c[mc], err_b_t[mc], err_f_t[mc], T_EDGES),
            ("wind", exc_w[mw], err_b_w[mw], err_f_w[mw], W_EDGES),
        ]
        for col_offset in [0, 3]:
            metric = "rmse" if col_offset == 0 else "bias"
            for j, (name, exc, eb, ef, edges) in enumerate(data_panels):
                ax = axes[row, col_offset + j]
                cb, rb, bib, ctr = bin_stats(exc, eb, edges)
                cf, rf, bif, _   = bin_stats(exc, ef, edges)
                yb = rb if metric == "rmse" else bib
                yf = rf if metric == "rmse" else bif
                ax.plot(ctr, yb, "o-", color="gray", lw=1.8, ms=5, label="GC base")
                ax.plot(ctr, yf, "D-", color=color, lw=1.8, ms=4, label=model_name)
                if metric == "bias":
                    ax.axhline(0, color="k", lw=0.5)
                if row == 0:
                    ax.set_title(panels_template[col_offset + j], fontsize=9)
                if col_offset + j == 0:
                    ax.set_ylabel(f"lead K={K_lead} ({lead_h}h)", fontsize=10)
                ax.grid(alpha=0.3); ax.tick_params(labelsize=7)
                if row == 0 and col_offset + j == 0:
                    ax.legend(fontsize=7)
    fig.suptitle(f"{model_name} extreme-records: 2022 events vs 2015-2021 record (res=2)",
                 fontsize=12, y=1.00)
    plt.tight_layout()
    plt.savefig(out_path, dpi=140, bbox_inches="tight"); plt.close(fig)


def plot_overlay(preds_v25, preds_v22, recs, out_path: Path):
    """Same 6×6 grid, but show v22 K=18 (baseline) and v25 (gated extreme) ON TOP of GC base."""
    panels_template = ["Heat RMSE", "Cold RMSE", "Wind RMSE", "Heat bias", "Cold bias", "Wind bias"]
    fig, axes = plt.subplots(len(LEAD_K), 6, figsize=(22, 3.0*len(LEAD_K)))

    def _av(preds, name, k_idx):
        return preds[name].isel(lead_h=k_idx).transpose("anchor_time", "lat", "lon").values

    for row, K_lead in enumerate(LEAD_K):
        k_idx = K_lead - 1
        lead_h = K_lead * 6
        # Use v25's anchor times (assume identical to v22's; if not, will subset)
        t_tru = _av(preds_v25, "2m_temperature_truth", k_idx)
        u_tru = _av(preds_v25, "10m_u_component_of_wind_truth", k_idx)
        v_tru = _av(preds_v25, "10m_v_component_of_wind_truth", k_idx)
        w_tru = np.sqrt(u_tru**2 + v_tru**2)
        t_b = _av(preds_v25, "2m_temperature_baseline", k_idx)
        t_v25 = _av(preds_v25, "2m_temperature_v18", k_idx)
        t_v22 = _av(preds_v22, "2m_temperature_v18", k_idx)
        u_b = _av(preds_v25, "10m_u_component_of_wind_baseline", k_idx)
        v_b = _av(preds_v25, "10m_v_component_of_wind_baseline", k_idx)
        u_v25 = _av(preds_v25, "10m_u_component_of_wind_v18", k_idx)
        v_v25 = _av(preds_v25, "10m_v_component_of_wind_v18", k_idx)
        u_v22 = _av(preds_v22, "10m_u_component_of_wind_v18", k_idx)
        v_v22 = _av(preds_v22, "10m_v_component_of_wind_v18", k_idx)
        w_b   = np.sqrt(u_b**2 + v_b**2)
        w_v25 = np.sqrt(u_v25**2 + v_v25**2)
        w_v22 = np.sqrt(u_v22**2 + v_v22**2)

        anc_times = preds_v25.anchor_time.values
        valid_times = anc_times + np.timedelta64(lead_h, "h")
        doys = pd.DatetimeIndex(valid_times).dayofyear.values
        rmx  = recs["t2m_record_max"].sel(doy=xr.DataArray(doys, dims="anchor_time")).transpose("anchor_time", "lat", "lon").values
        rmn  = recs["t2m_record_min"].sel(doy=xr.DataArray(doys, dims="anchor_time")).transpose("anchor_time", "lat", "lon").values
        rwmx = recs["wind10m_record_max"].sel(doy=xr.DataArray(doys, dims="anchor_time")).transpose("anchor_time", "lat", "lon").values

        exc_h = (t_tru - rmx).ravel()
        err_b_t   = (t_b   - t_tru).ravel()
        err_v25_t = (t_v25 - t_tru).ravel()
        err_v22_t = (t_v22 - t_tru).ravel()
        exc_c = (rmn - t_tru).ravel()
        exc_w = (w_tru - rwmx).ravel()
        err_b_w   = (w_b   - w_tru).ravel()
        err_v25_w = (w_v25 - w_tru).ravel()
        err_v22_w = (w_v22 - w_tru).ravel()
        mh = exc_h > 0; mc = exc_c > 0; mw = exc_w > 0
        data_panels = [
            ("heat", exc_h[mh], err_b_t[mh],   err_v25_t[mh], err_v22_t[mh], T_EDGES),
            ("cold", exc_c[mc], err_b_t[mc],   err_v25_t[mc], err_v22_t[mc], T_EDGES),
            ("wind", exc_w[mw], err_b_w[mw],   err_v25_w[mw], err_v22_w[mw], W_EDGES),
        ]
        for col_offset in [0, 3]:
            metric = "rmse" if col_offset == 0 else "bias"
            for j, (name, exc, eb, e25, e22, edges) in enumerate(data_panels):
                ax = axes[row, col_offset + j]
                _, rb, bib, ctr = bin_stats(exc, eb,  edges)
                _, r25, b25, _ = bin_stats(exc, e25, edges)
                _, r22, b22, _ = bin_stats(exc, e22, edges)
                yb  = rb  if metric == "rmse" else bib
                y25 = r25 if metric == "rmse" else b25
                y22 = r22 if metric == "rmse" else b22
                ax.plot(ctr, yb,  "o-", color="gray", lw=1.8, ms=5, label="GC base")
                ax.plot(ctr, y22, "s-", color="C0",   lw=1.5, ms=4, label="v22 K=18")
                ax.plot(ctr, y25, "D-", color="C2",   lw=1.8, ms=4, label="v25 step3k")
                if metric == "bias":
                    ax.axhline(0, color="k", lw=0.5)
                if row == 0:
                    ax.set_title(panels_template[col_offset + j], fontsize=9)
                if col_offset + j == 0:
                    ax.set_ylabel(f"lead K={K_lead} ({lead_h}h)", fontsize=10)
                ax.grid(alpha=0.3); ax.tick_params(labelsize=7)
                if row == 0 and col_offset + j == 0:
                    ax.legend(fontsize=7)
    fig.suptitle("v22 K=18 vs v25 gated-extreme step3000 — extreme-records 2022 (res=2)",
                 fontsize=12, y=1.00)
    plt.tight_layout()
    plt.savefig(out_path, dpi=140, bbox_inches="tight"); plt.close(fig)


def main():
    recs = xr.open_dataset(RECS_RES2)
    v25_p = DATA_DIR / "v25_step3000_res2_extreme_records_K40_2022.nc"
    v22_p = DATA_DIR / "v22_K18_step20000_res2_extreme_records_K40_2022.nc"

    if v25_p.exists():
        ds_v25 = xr.open_dataset(v25_p)
        plot_one_model(ds_v25, recs, "v25 step3k (gated extreme)", "C2",
                        OUT_DIR / "extreme_records_v25_step3000.png")
        print(f"saved {OUT_DIR / 'extreme_records_v25_step3000.png'}")
    else:
        print(f"[skip] {v25_p} not yet ready")

    if v22_p.exists():
        ds_v22 = xr.open_dataset(v22_p)
        plot_one_model(ds_v22, recs, "v22 K=18 step20k", "C0",
                        OUT_DIR / "extreme_records_v22_K18.png")
        print(f"saved {OUT_DIR / 'extreme_records_v22_K18.png'}")

    if v25_p.exists() and v22_p.exists():
        plot_overlay(ds_v25, ds_v22, recs,
                     OUT_DIR / "extreme_records_v25_vs_v22.png")
        print(f"saved {OUT_DIR / 'extreme_records_v25_vs_v22.png'}")


if __name__ == "__main__":
    main()
