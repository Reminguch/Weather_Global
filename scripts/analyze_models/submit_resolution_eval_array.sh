...
DISCOVER_CMD=(python -u scripts/analyze_models/unified_resolution_eval.py --families ${FAMILIES} --data-source "${DATA_SOURCE}" --print-shards)
...
if [[ "${#RES1_SHARDS[@]}" -gt 0 ]]; then
  ...
  RES1_JOB_ID=$(
    sbatch \
      ...
      --export=ALL,SHARD_SPECS="${RES1_SPECS}",WARMUP_STEPS="${WARMUP_STEPS}",TRUNK_STEPS="${TRUNK_STEPS}",METRICS="${METRICS}",EVAL_MODES="${EVAL_MODES}",CHECKPOINT_ROOTS="${CHECKPOINT_ROOTS}",CHECKPOINT_PATHS="${CHECKPOINT_PATHS}",DATA_SOURCE="${DATA_SOURCE}",EVAL_YEAR="${EVAL_YEAR}",RESIDUAL_EVAL_SEMANTICS="${RESIDUAL_EVAL_SEMANTICS}" \
...
if [[ "${#OTHER_SHARDS[@]}" -gt 0 ]]; then
  ...
  ARRAY_JOB_ID=$(
    sbatch \
      ...
      --time="${DEFAULT_ARRAY_TIME}" \
      --array="0-${ARRAY_MAX}" \
      --export=ALL,SHARD_SPECS="${OTHER_SPECS}",WARMUP_STEPS="${WARMUP_STEPS}",TRUNK_STEPS="${TRUNK_STEPS}",METRICS="${METRICS}",EVAL_MODES="${EVAL_MODES}",CHECKPOINT_ROOTS="${CHECKPOINT_ROOTS}",CHECKPOINT_PATHS="${CHECKPOINT_PATHS}",DATA_SOURCE="${DATA_SOURCE}",EVAL_YEAR="${EVAL_YEAR}",RESIDUAL_EVAL_SEMANTICS="${RESIDUAL_EVAL_SEMANTICS}" \
      scripts/analyze_models/run_resolution_eval_array.slurm
  )
