...
EXPERIMENT="res2_ds16_gc_mamba_release_all20k_warm_leads1_9d"
...
MEM="${RES2_DS16_RELEASE_EVAL_MEM:-48G}"
TIME="${RES2_DS16_RELEASE_EVAL_TIME:-01:30:00}"
...
RELEASE_ROOT="artifacts/checkpoints/7_years/mamba_release_all_from_frozen20k_res2_ds16_bptt16_20k"
BASE_NAME="vanilla_gc_7y_res2_m4_w512_mp6_h6_bs8_accum1_stream50k"

declare -a EVAL_SPECS=()
for target_steps in 4 8 12; do
  for d_inner in 16 64 256; do
    run="${BASE_NAME}_gc_mamba_tc2_di${d_inner}_ds16_20k_target_step${target_steps}_bptt16"
    EVAL_SPECS+=(
      "gc_mamba release_di${d_inner}_ds16_ts${target_steps} ${RELEASE_ROOT}/${run}_release_all20k/ckpt_best.npz"
    )
  done
done
for target_steps in 4 8 12; do
  for d_inner in 16 64 256; do
    run="${BASE_NAME}_gc_mamba_tc2_di${d_inner}_ds16_20k_target_step${target_steps}_bptt16"
    EVAL_SPECS+=(
      "gc_mamba frozen_di${d_inner}_ds16_ts${target_steps} ${FROZEN_ROOT}/${run}/ckpt_best.npz"
    )
  done
done
EVAL_SPECS+=(
...
if [[ "${#EVAL_SPECS[@]}" -ne 20 ]]; then
  echo "Expected 20 eval specs, got ${#EVAL_SPECS[@]}" >&2
  exit 1
fi
...
for spec in "${EVAL_SPECS[@]}"; do
  ...
  if [[ ! -f "${ckpt}" ]]; then
    ...
    elif [[ -n "${UPSTREAM_DEPENDENCY}" && "${label}" == release_* ]]; then
      echo "Deferring missing release checkpoint for ${label} until after dependency ${UPSTREAM_DEPENDENCY}: ${ckpt}" >&2
    else
