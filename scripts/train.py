#!/usr/bin/env python3
"""Train entrypoint for one-batch smoke and quick experiments.
...
from src.pipelines.train import initialize_training, train_one_step
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run model-agnostic training loop.")
    ...
    parser.add_argument("--steps", type=int, default=1, help="Number of train steps.")
    ...
    model, state = initialize_training(config, rng=rng, sample_batch=batch)

    last_metrics: Mapping[str, Any] = {}
    last_predictions: Mapping[str, Any] = {}
    for _ in range(args.steps):
        state, last_metrics, last_predictions = train_one_step(model, state, batch)
        loss = _to_float(last_metrics.get("loss", 0.0))
        print(f"step={state.step} loss={loss:.6f}")

    if args.predictions_out:
        _save_predictions(args.predictions_out, last_predictions)
