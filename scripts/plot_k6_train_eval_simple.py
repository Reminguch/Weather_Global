#!/usr/bin/env python3
"""Simple plot: K=6 best training loss + eval loss (TF mode) of K=6 phase only
(steps 6000..8000), since K=6 was resumed from K=4 step 6000."""
...
RUN_DIR = Path("/home/lm8598/Weather_Global_experiments/results/mz_residual_memory/"
               "mz_fullmamba_paperckpt_r1_in2_seg16_meshed_m5_h128_ds16_fullvars_K6_8k")
K_PHASE_START = 6000
...
train_k6 = [e for e in train if e["step"] > K_PHASE_START]
ev_k6 = [e for e in ev if e["step"] > K_PHASE_START]

train_steps = np.array([e["step"] for e in train_k6])
train_losses = np.array([e.get("loss", e.get("total_loss", 0)) for e in train_k6])

ev_steps = np.array([e["step"] for e in ev_k6])
ev_corrected_loss = np.array([e["total_loss"] for e in ev_k6])
ev_baseline_mae = np.array([e["baseline_overall_MAE"] for e in ev_k6])
ev_corrected_mae = np.array([e["corrected_overall_MAE"] for e in ev_k6])
...
ax.set_title("K=6 best (FullMamba S6, segment=8, target_steps=6)\n"
             "K=6 phase: step 6000 → 8000 (resumed from K=4 step 6000)")
ax.grid(alpha=0.3)
ax.legend(loc="upper right")

OUT = Path("/home/lm8598/Weather_Global_experiments/results/2026-04-27_train_eval_curves")
OUT.mkdir(parents=True, exist_ok=True)
out_path = OUT / "k6_train_eval_simple.png"
plt.tight_layout()
