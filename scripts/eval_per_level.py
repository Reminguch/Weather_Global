...
def main():
    ...
    p.add_argument(
        "--warm-start-cycles", type=int, default=0,
        help="Number of past observed cycles to use to warm up the SSM "
             "hidden state BEFORE launching the K-step forecast rollout. "
             "0 (default) = cold-start (h_0 = 0 at each anchor's t=0). "
             "Recommended for K=1-trained ckpts where SSM hidden was "
             "trained to propagate across consecutive cycles within a "
             "segment of length segment_steps. Setting --warm-start-cycles "
             "16 (matching K=1 v3 training segment_steps) lets eval use "
             "the same continuous-cycle hidden-state regime as training. "
             "Warm-start uses ONLY data at times <= forecast launch (real "
             "ERA5 obs of past cycles + their observable previous-cycle "
             "residuals), so it does not leak future truth.",
    )
    p.add_argument(
    ...
    # Warm-start phase always uses anchor-as-batch layout ([T=W, B=S]) to
    # gather W consecutive warm cycles per anchor in parallel. If forecast
    # phase falls back to legacy layout ([T=S*K, B=1]) the SSM hidden state
    # produced by warm phase has B=S which doesn't broadcast onto the
    # forecast's B=1 SSM call -> shape mismatch (M vs S*M). Force
    # effective_anchor_as_batch=True whenever --warm-start-cycles > 0 so
    # both phases share the same B axis.
    if int(getattr(args, "warm_start_cycles", 0)) > 0 and not effective_anchor_as_batch:
        print(
            f"[eval] --warm-start-cycles={args.warm_start_cycles} forces "
            f"effective_anchor_as_batch=True (was False with horizon={horizon} < "
            f"min_k={cfg_anchor_as_batch_min_k}) so warm + forecast phases "
            f"share the [T, B=S] layout."
        )
        effective_anchor_as_batch = True
    print(
    ...
    def residual_objective_ar_warm(
        seq_inputs, baseline_next, truth_next,
        warm_seq_inputs, warm_baseline_next, warm_tf_mask,
        is_training,
    ):
        """AR forecast with warm-start hidden state: identical to TF-warm,
        except the forecast phase is pure AR (no truth feedback). Warm phase
        still teacher-forced over W observed cycles to seed h."""
        m = mk()
        # WARM-START PHASE (same as TF-warm)
        warm_cs_n, warm_tpr_n = _normalize_inputs(warm_seq_inputs)
        warm_baseline_abs_n = (
            warm_baseline_next - input_mean_f[None, None, None, None, :]
        ) / input_std_f[None, None, None, None, :]
        _, h_warm = m.rollout_ar(
            warm_cs_n,
            is_training=is_training,
            true_prev_residual_n_tblnf=warm_tpr_n,
            tf_mask_per_step=warm_tf_mask,
            baseline_absolute_n_tblnf=warm_baseline_abs_n,
            residual_to_state_rescale_f=residual_to_state_rescale_f,
            allow_tf_at_t0=True,
            return_final_h_states=True,
        )
        # AR FORECAST PHASE (initial_h_states=h_warm; no true_prev_residual)
        cs_n, _ = _normalize_inputs(seq_inputs)
        baseline_abs_n = (
            baseline_next - input_mean_f[None, None, None, None, :]
        ) / input_std_f[None, None, None, None, :]
        pred_residual_n = m.rollout_ar(
            ...
            zero_prev_residual_input=args.zero_prev_residual,
            disable_state_feedback=args.disable_state_feedback,
            initial_h_states=h_warm,
        )
        pred = _pred_to_real(pred_residual_n)
        return {"corrected": baseline_next + pred}

    # Warm-start variants: when --warm-start-cycles W > 0, run an additional
    # rollout_ar phase over W observed past cycles BEFORE the forecast launch
    # to evolve the SSM hidden state, then start the K-step forecast rollout
    # from that warmed state. Matches K=1-style training where SSM hidden
    # propagates across consecutive cycles within a segment.
    W = int(args.warm_start_cycles)

    def residual_objective_tf_warm(
        seq_inputs, baseline_next, truth_next, tf_mask,
        warm_seq_inputs, warm_baseline_next, warm_tf_mask,
        is_training,
    ):
        """K-step TF rollout with warm-start: a separate rollout_ar phase
        over W observed cycles seeds the SSM hidden state before the
        forecast phase. Warm phase uses tf_mask=ones (all real obs); no
        ablation flags applied (warm-start is for hidden init, not testing).
        """
        m = mk()
        # --- WARM-START PHASE ---
        warm_cs_n, warm_tpr_n = _normalize_inputs(warm_seq_inputs)
        ...
        _, h_warm = m.rollout_ar(
            ...
            baseline_absolute_n_tblnf=warm_baseline_abs_n,
            residual_to_state_rescale_f=residual_to_state_rescale_f,
            allow_tf_at_t0=True,           # warm-start: every step is real obs
            return_final_h_states=True,
        )
        # --- FORECAST PHASE (K-step AR with warmed h_states) ---
    ...
    if args.mode == "tf":
        if W > 0:
            residual_model = hk.transform(residual_objective_tf_warm)
        else:
            residual_model = hk.transform(residual_objective_tf)
    else:
        if W > 0:
            residual_model = hk.transform(residual_objective_ar_warm)
        else:
    ...
    if args.mode == "tf":
        if W > 0:
            @jax.jit
            def infer_step(p, k, si, bn, tn, tfm, wsi, wbn, wtm):
                return residual_model.apply(p, k, si, bn, tn, tfm, wsi, wbn, wtm, False)
        else:
            @jax.jit
        ...
        if W > 0:
            @jax.jit
            def infer_step(p, k, si, bn, tn, tfm, wsi, wbn, wtm):  # tfm unused
                return residual_model.apply(p, k, si, bn, tn, wsi, wbn, wtm, False)
        else:
            @jax.jit
    ...
    # Warm-start: filter eval_segments so [seg[0]-W .. seg[0]-1] is a valid
    # contiguous range in the eval dataset (drop segments at the very start
    # that lack W frames of history).
    if W > 0:
        n_before = len(eval_segments)
        eval_segments = [s for s in eval_segments if int(s[0]) - W >= 0]
        # Also enforce that all extended indices are in the time-continuous
        # eval index set (avoid year-boundary leaks).
        eval_idx_set = set(int(x) for x in eval_indices.tolist())
        eval_segments = [
            s for s in eval_segments
            if all(int(s[0]) - W + j in eval_idx_set
                   for j in range(W))
        ]
        print(
            f"[eval] warm_start_cycles={W}: kept {len(eval_segments)}/{n_before} "
            f"segments after filtering for valid W-frame history."
        )

    # Pre-compute warm gather indices: warm[w, s] = ext[s + w]
    warm_gather_W_S = np.array(
        [[s + w for s in range(0)] for w in range(W)],   # placeholder, redone per-seg
        dtype=np.int64
    )

    for i, seg in enumerate(eval_segments):
        rng, key = jax.random.split(rng)
        # Forecast tensors at the eval segment.
        ...
        if W > 0:
            # Warm tensors: extend segment backward by W indices, build a
            # legacy K=1 batch covering [seg[0]-W .. seg[-1]] with one
            # 1-step prediction per anchor. Then gather warm[w, s] = ext[s+w]
            # so each eval anchor s gets W consecutive warm-cycles ending at
            # the obs immediately before its forecast launch.
            #
            # CHUNKING: _segment_to_tensors batches the baseline GraphCast
            # forward over len(seg) anchors. For S+W >= 18 this OOMs on
            # gpu80 because each anchor pulls a full GraphCast scan. Process
            # the extended segment in chunks of <= BATCH_CHUNK anchors and
            # concatenate; preserves time-continuous validation per chunk.
            S_seg = len(seg)
            ext_seg = np.arange(int(seg[0]) - W, int(seg[0]) + S_seg,
                                dtype=seg.dtype)
            BATCH_CHUNK = 8
            chunk_starts = list(range(0, len(ext_seg), BATCH_CHUNK))
            chunks = []
            for cs in chunk_starts:
                chunks.append(ext_seg[cs:cs + BATCH_CHUNK])
            # Merge any trailing 1-element chunk into the previous one so
            # _segment_to_tensors's len>=2 validator is happy.
            if len(chunks) >= 2 and len(chunks[-1]) < 2:
                chunks[-2] = np.concatenate([chunks[-2], chunks[-1]])
                chunks = chunks[:-1]
            ext_inputs_parts, ext_baseline_parts = [], []
            for chunk in chunks:
                rng, wkey = jax.random.split(rng)
                ci, cb, _ct, _tfm = mz_train._segment_to_tensors(
                    baseline_predict, baseline_params, baseline_state, wkey,
                    eval_ds, chunk, input_steps=input_steps, task_cfg=task_cfg,
                    dt=dt, feature_order=feature_order, target_steps=1,
                    anchor_as_batch=False)
                # ci: [chunk_len, B=1, lat, lon, 2F]; pull to numpy to free
                # GPU memory between chunks.
                ext_inputs_parts.append(np.asarray(ci, dtype=np.float32))
                ext_baseline_parts.append(np.asarray(cb, dtype=np.float32))
            # Concatenate along the time/anchor axis; drop the B=1 axis.
            ext_inputs_full = np.concatenate(ext_inputs_parts, axis=0)
            ext_baseline_full = np.concatenate(ext_baseline_parts, axis=0)
            ext_inputs_sq = ext_inputs_full[:, 0, ...]       # [S+W, lat, lon, 2F]
            ext_baseline_sq = ext_baseline_full[:, 0, ...]    # [S+W, lat, lon, F]
            # Gather warm[w, s] = ext[s+w]
            gather_idx = np.array(
                [[s + w for s in range(S_seg)] for w in range(W)],
                dtype=np.int64
            )  # [W, S]
            warm_seq_inputs = jnp.asarray(ext_inputs_sq[gather_idx])      # [W, S, lat, lon, 2F]
            warm_baseline_next = jnp.asarray(ext_baseline_sq[gather_idx]) # [W, S, lat, lon, F]
            warm_tf_mask = jnp.ones((W,), dtype=jnp.float32)
            out = infer_step(
                mem_params, key,
                seq_inputs, baseline_next, truth_next, tf_mask,
                warm_seq_inputs, warm_baseline_next, warm_tf_mask,
