...
from src.models.mamba.v24_Ilya import anchor_manifest
from src.models.mamba.v24_Ilya.training.data import build_segments
...
def test_january_manifest_requires_empty_validation_opt_in_and_has_five_chunks(
    ...
    verified = anchor_manifest.build_anchor_manifest(
        prepared_root=prepared_root,
        baseline_checkpoint=Path("graphcast37.npz"),
        output_root=verified_root,
        allow_empty_validation=True,
        reference_root=output_root,
    )
    assert verified["architecture_id"] == "v24_Ilya"
    assert verified["reference_arrays_verified_equal"] is True
