...
from scripts.analyze_models.merge_v23_Ilya_eval_shards import (
    merge_v23_Ilya_eval_shards,
)
from src.models.mamba.v23_Ilya.evaluation import (
    build_evaluation_result,
    compare_metric_outputs,
    partial_result_path,
    write_evaluation_snapshot,
)
from src.models.mamba.v23_Ilya.config import V23IlyaEvalConfig
from src.models.mamba.v23_Ilya import data as evaluation_data
from src.models.mamba.v23_Ilya.data import EvalDataset, valid_scored_eval_indices
...
def test_metric_comparison_includes_original_graphcast_loss() -> None:
    common = {
        "chosen_idx": [1],
        "original_graphcast_loss": {"baseline_rollout": 2.0},
        "per_variable_per_step": {},
    ...
    candidate = {
        **common,
        "original_graphcast_loss": {"baseline_rollout": 3.0},
    }
    with pytest.raises(AssertionError):
        compare_metric_outputs(common, candidate, rtol=1e-5, atol=1e-6)
...
def test_eval_config_validates_anchor_shards(tmp_path) -> None:
    ...
    config = V23IlyaEvalConfig(**common, anchor_shard_index=3)
    ...
    with pytest.raises(ValueError, match="anchor_shard_index"):
        V23IlyaEvalConfig(**common, anchor_shard_index=4)
    with pytest.raises(ValueError, match="force_idx cannot"):
        V23IlyaEvalConfig(**common, anchor_shard_index=0, force_idx=2)


def test_eval_config_resolves_and_validates_metric_backends(tmp_path) -> None:
    common = {
        "ckpt": tmp_path / "checkpoint.pkl",
        "out_json": tmp_path / "output.json",
        "eval_mode": "warm_full",
    }
    assert V23IlyaEvalConfig(**common).resolved_metric_backend == "cpu"
    assert (
        V23IlyaEvalConfig(**common, omit_rms_bias=True).resolved_metric_backend
        == "device"
    )
    assert (
        V23IlyaEvalConfig(
            **common,
            omit_rms_bias=True,
            metric_backend="cpu",
        ).resolved_metric_backend
        == "cpu"
    )
    with pytest.raises(ValueError, match="requires omit_rms_bias=True"):
        V23IlyaEvalConfig(**common, metric_backend="device")
    with pytest.raises(ValueError, match="metric_backend must be one of"):
        V23IlyaEvalConfig(**common, metric_backend="unknown")
...
def test_merge_script_reconstructs_exact_metrics_and_removes_states(tmp_path) -> None:
    ...
    shard_json_paths = []
    merge_state_paths = []
    monolithic = V23IlyaMetricAccumulator(
        target_steps=1,
    ...
    for shard_index, anchor_index in enumerate(global_indices):
        ...
        base_output = {
            ...
            "metric_backend_requested": "auto",
            "metric_backend": "device",
            "n_samples": 4,
            "anchor_shard_count": 4,
    ...
    mismatched = json.loads(shard_json_paths[1].read_text(encoding="utf-8"))
    mismatched["metric_backend"] = "cpu"
    shard_json_paths[1].write_text(json.dumps(mismatched), encoding="utf-8")
    with pytest.raises(ValueError, match="metadata differs for 'metric_backend'"):
        merge_v23_Ilya_eval_shards(
            shard_json_paths=shard_json_paths,
            merge_state_paths=merge_state_paths,
            out_json=tmp_path / "rejected.json",
            delete_merge_states=False,
        )
    mismatched["metric_backend"] = "device"
    shard_json_paths[1].write_text(json.dumps(mismatched), encoding="utf-8")

    output_path = tmp_path / "merged.json"
    output = merge_v23_Ilya_eval_shards(
        shard_json_paths=shard_json_paths,
        merge_state_paths=merge_state_paths,
