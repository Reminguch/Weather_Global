...
from src.models.mamba.v23_Ilya.metrics import (
    ORIGINAL_GRAPHCAST_VARIABLE_WEIGHTS,
    V23IlyaDeviceMetricReducer,
    V23IlyaMetricAccumulator,
)
...
def test_leadwise_metrics_match_batched_updates_across_samples() -> None:
    ...
    batched = V23IlyaMetricAccumulator(target_steps=2, latitudes=np.asarray([0.0]))
    streamed = V23IlyaMetricAccumulator(target_steps=2, latitudes=np.asarray([0.0]))
...
def test_streamed_original_graphcast_loss_matches_graphcast_implementation() -> None:
    ...
    accumulator = V23IlyaMetricAccumulator(
        target_steps=2,
...
def test_batched_and_leadwise_original_graphcast_loss_match() -> None:
    ...
    batched = V23IlyaMetricAccumulator(**common)
    streamed = V23IlyaMetricAccumulator(**common)
    batched.update(truth, baseline, full)
    for step_index in range(2):
        selection = {"time": slice(step_index, step_index + 1)}
        streamed.update_step(
...
def test_exported_metric_states_merge_to_monolithic_result() -> None:
    ...
    monolithic = V23IlyaMetricAccumulator(**common)
    shards = [V23IlyaMetricAccumulator(**common) for _ in range(4)]
    ...
    merged = V23IlyaMetricAccumulator.from_merge_state(shards[0].export_merge_state())
...
def test_metric_merge_rejects_different_latitudes() -> None:
    first = V23IlyaMetricAccumulator(target_steps=1, latitudes=np.asarray([0.0]))
    second = V23IlyaMetricAccumulator(target_steps=1, latitudes=np.asarray([10.0]))
    truth = _dataset([0.0])
...
def test_lightweight_metrics_skip_spatial_bias_and_merge_exactly() -> None:
    ...
    first = V23IlyaMetricAccumulator(
        target_steps=2,
        latitudes=np.asarray([0.0]),
        store_spatial_bias=False,
    )
    second = V23IlyaMetricAccumulator(
        target_steps=2,
        latitudes=np.asarray([0.0]),
        store_spatial_bias=False,
    )
    ...
    merged = V23IlyaMetricAccumulator.from_merge_state(first.export_merge_state())
...
def test_device_metrics_match_cpu_for_surface_pressure_and_pole_weights() -> None:
    coordinates = {
        "batch": [0],
        "time": [0, 1],
        "level": [500, 1000],
    ...
    cpu = V23IlyaMetricAccumulator(**common)
    device = V23IlyaMetricAccumulator(**common)
    reducer = V23IlyaDeviceMetricReducer(device)
    ...
    for sample_index in range(2):
        truth = xr.Dataset(
            {
                "2m_temperature": (
                    ("batch", "time", "lat", "lon"),
                    np.full((1, 2, 3, 2), sample_index, dtype=np.float32),
                ),
                "temperature": (
                    ("batch", "time", "level", "lat", "lon"),
                    np.full((1, 2, 2, 3, 2), sample_index, dtype=np.float32),
                ),
            },
            coords=coordinates,
        )
        latitude_pattern = np.asarray(
            [-2.0, 0.5, 3.0],
            dtype=np.float32,
        ).reshape(1, 1, 3, 1)
        surface_error = np.broadcast_to(
            latitude_pattern * (sample_index + 1),
            (1, 2, 3, 2),
        ).copy()
        pressure_error = np.broadcast_to(
            surface_error[:, :, None, :, :]
            * np.asarray([1.0, 2.0], dtype=np.float32).reshape(1, 1, 2, 1, 1),
            (1, 2, 2, 3, 2),
        ).copy()
        baseline = truth + xr.Dataset(
            {
        ...
        full = truth + (baseline - truth) * (0.25 + 0.1 * sample_index)

        cpu.update(truth, baseline, full)
        reducer.begin_sample()
        for step_index in range(2):
            selection = {"time": slice(step_index, step_index + 1)}
            reducer.update_step(
                step_index,
    ...
    expected = cpu.finalize()
    actual = device.finalize()
    ...
    cpu_state = cpu.export_merge_state()
    device_state = device.export_merge_state()
    assert cpu_state["format"] == device_state["format"]
    assert cpu_state["stores_spatial_bias"] is False
    assert device_state["stores_spatial_bias"] is False
    assert set(cpu_state["arrays"]) == set(device_state["arrays"])
    restored = V23IlyaMetricAccumulator.from_merge_state(device_state)
    assert_nested_close(actual, restored.finalize())


def test_device_metrics_reject_spatial_bias_accumulator() -> None:
    accumulator = V23IlyaMetricAccumulator(
        target_steps=1,
        latitudes=np.asarray([0.0]),
        store_spatial_bias=True,
    )
    with np.testing.assert_raises_regex(ValueError, "store_spatial_bias=False"):
        V23IlyaDeviceMetricReducer(accumulator)
